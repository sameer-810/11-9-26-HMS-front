...
import { X, Info, TriangleAlert, CircleCheck, CircleAlert } from "lucide-react-native";
...
export function Banner({ tone = "info", title, message, action, onDismiss, style }: Props) {
  ...
  return (
    <View
      ...
      style={[styles.wrap, { backgroundColor: t.bg, borderColor: t.border }, style]}
    >
      <HStack gap={10} align="flex-start">
        <Icon size={18} color={t.text} strokeWidth={2.2} style={{ marginTop: 1 }} />
        <VStack gap={2} flex={1}>
          {title ? (
