      ...
      <VStack gap={14}>
        {bill.status === "cancelled" ? (
          <Banner
            ...
            message={`${bill.cancelReason}. Its charges were released for another bill.`}
          />
        ) : null}
        ...
        <Card testID="bill-charges">
          <VStack gap={10}>
            <SectionHeader
              ...
              subtitle={
                draft
                  ? "Draft — charges can still change"
                  : "Fixed when the bill was finalised"
              }
              right={
                !draft ? (
                  <Lock size={15} color={palette.text.tertiary} />
                ) : undefined
              }
            />
            {bill.hasUnpriced ? (
              <Banner
                tone="warning"
                title="A charge has no price"
                message="Administration must price it before this bill can be finalised. Refresh the bill once it is priced."
              />
            ) : null}
            <ChargeTable
              lines={bill.lines}
              onRemove={draft && canManage ? setRemoving : undefined}
              testID="bill-charge-table"
            />
            {removing ? (
              <RemoveLine
                bill={bill}
              ...
              <VStack gap={2}>
                {bill.removedLines.map((r, i) => (
                  <Text key={i} variant="caption" tone="tertiary">
                    Removed {r.description} ({formatRupees(r.amount)}) —{" "}
                    {r.reason} · {r.byName}
                  </Text>
                ))}
              </VStack>
        ...
        {draft && canApprove && bill.discount.status === "pending" ? (
          <DiscountDecision bill={bill} />
        ) : null}
        {payable && canPay ? (
      ...
      <VStack gap={6}>
        ...
        {d.status !== "none" ? (
          <Row
            ...
            value={
              d.status === "approved"
                ? `− ${formatRupees(bill.discountAmount)}`
...
function Row({
  ...
  return (
    <HStack justify="space-between" align="center">
      <Text
        variant={strong ? "label" : "body-sm"}
        tone={muted ? "tertiary" : "secondary"}
      >
        {label}
      </Text>
      <Text
        variant={strong ? "label-lg" : "body-sm"}
        tabular
...
function RemoveLine({
  ...
  return (
    <VStack gap={8} style={styles.subform} testID="remove-line-form">
      ...
      <TextField
        label="Why"
        hint="The charge is released and will be compiled onto the next bill."
        value={reason}
        onChangeText={setReason}
        testID="remove-reason"
      />
      {remove.isError ? (
        <Banner tone="danger" message={apiErrorMessage(remove.error)} />
      ) : null}
      <HStack gap={8}>
        <Button
          label="Remove charge"
...
function DraftActions({ bill }: { bill: Bill }) {
  ...
  const error = [refresh, addService, discount, finalise, cancel].find(
    (m) => m.isError,
  )?.error;
  ...
  return (
    <Card testID="draft-actions">
      <VStack gap={14}>
        <SectionHeader
          title="Draft"
          subtitle="Check the charges, then finalise"
        />
        {error ? (
        ...
        <HStack gap={8} wrap>
          <Button
            label="Refresh charges"
            size="sm"
            variant="secondary"
            loading={refresh.isPending}
            onPress={() => refresh.mutate(undefined as never)}
            testID="refresh-bill"
          />
        </HStack>
        ...
        <VStack gap={8}>
          ...
          <HStack gap={8} wrap align="flex-end">
            <View style={{ flex: 1, minWidth: 220 }}>
              <Select
                ...
                options={tariff.map((t) => ({
                  value: t.id,
                  label: t.name,
                  sublabel: formatRupees(t.price),
                }))}
          ...
          <Text variant="caption" tone="tertiary">
            Prices come from the tariff set by administration.
          </Text>
        </VStack>

        {bill.discount.status === "none" ||
        bill.discount.status === "rejected" ? (
          <VStack gap={8}>
            <Text variant="label">Request a discount</Text>
            {bill.discount.status === "rejected" ? (
              <Text variant="caption" tone="warning">
                The last request was rejected by {bill.discount.decidedByName}:{" "}
                {bill.discount.decisionNote || "no note"}
              </Text>
            ) : null}
            <HStack gap={8} wrap>
              <TextField
            ...
            />
            <Text variant="caption" tone="tertiary">
              A discount is applied only once someone else approves it.
            </Text>
          </VStack>
        ) : bill.discount.status === "pending" ? (
          <Text variant="caption" tone="warning" testID="discount-pending">
            Discount of {formatRupees(bill.discount.amount)} waiting for
            approval — {bill.discount.reason}
          </Text>
        ) : null}
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          finalise.mutate(undefined as never, {
    ...
    <Card testID="discount-decision">
      <VStack gap={10}>
        <SectionHeader
          ...
          subtitle={`Requested by ${bill.discount.requestedByName}`}
        />
        <Text variant="body">
        ...
        {decide.isError ? (
          <Banner tone="danger" message={apiErrorMessage(decide.error)} />
        ) : null}
        <HStack gap={8}>
          <Button
            label="Approve"
            ...
            onPress={() => decide.mutate({ approve: true, note })}
            testID="discount-approve"
          />
          <Button
            label="Reject"
            size="sm"
            variant="secondary"
            loading={decide.isPending}
            onPress={() => decide.mutate({ approve: false, note })}
            testID="discount-reject"
          />
        </HStack>
      </VStack>
    </Card>
...
function PaymentForm({ bill }: { bill: Bill }) {
  ...
  const ready =
    ...
    (method !== "cheque" ||
      (bankName.trim() && /^\d{4}-\d{2}-\d{2}$/.test(chequeDate)));
    ...
    <Card testID="payment-form">
      <VStack gap={10}>
        <SectionHeader
          ...
          subtitle={`Balance due ${formatRupees(bill.balanceDue)}`}
        />
        {pay.isError ? (
        ...
        <HStack gap={8} wrap>
          <TextField
            ...
            error={
              tooMuch
                ? `More than the balance of ${formatRupees(bill.balanceDue)}`
                : undefined
            }
      ...
      <VStack gap={0}>
        {payments.map((p) => (
          <View
            ...
            testID={`payment-${p.receiptNumber}`}
          >
            <HStack gap={10} align="center" wrap>
              <VStack gap={1} style={{ flex: 1, minWidth: 200 }}>
                ...
                <Text variant="caption" tone="tertiary">
                  ...
                  {p.isVoid
                    ? ` · VOID: ${p.voidReason} (${p.voidedByName})`
                    : ""}
                </Text>
              </VStack>
              <Button
                ...
                variant="secondary"
                onPress={() =>
                  navigation.navigate("Receipt", { paymentId: p.id })
                }
                testID={`receipt-${p.receiptNumber}`}
              />
              {canVoid && !p.isVoid ? (
                <Button
                  label="Void"
