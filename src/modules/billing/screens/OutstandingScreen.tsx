        ...
        <VStack gap={14}>
          <HStack gap={10} wrap>
            ...
            {BUCKETS.map((b) => (
              <StatTile
                key={b}
                label={data.buckets[b].label}
                value={formatRupees(data.buckets[b].amount)}
                attention={b === "90_plus" && data.buckets[b].amount > 0}
              />
            ))}
          </HStack>
          {data.rows.length === 0 ? (
            ...
            <VStack gap={8} testID="outstanding-rows">
              {rows.visible.map((r) => (
                <Card
                  ...
                  compact
                  onPress={() =>
                    navigation.navigate("BillDetail", { billId: r.id })
                  }
                  testID={`outstanding-${r.billNumber}`}
                >
                  <HStack gap={12} align="center" wrap>
                    <VStack gap={2} style={{ flex: 1, minWidth: 220 }}>
                      <Text variant="label">{r.patient.fullName}</Text>
                      <Text variant="caption" tone="secondary">
                        {r.patient.patientId} · {r.patient.mobile} ·{" "}
                        {r.billNumber}
                      </Text>
                    </VStack>
                    <VStack gap={2} align="flex-end">
                      <Text variant="label" tabular>
                        {formatRupees(r.balanceDue)} due
                      </Text>
                      <Text
                        variant="caption"
                        tone={r.bucket === "90_plus" ? "danger" : "tertiary"}
                      >
