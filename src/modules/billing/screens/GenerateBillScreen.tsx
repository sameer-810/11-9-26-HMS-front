...
export default function GenerateBillScreen() {
  ...
  const results = usePatients(
    !patientId && debounced.trim().length >= 2
  ...
  return (
    <Screen
      ...
      subtitle="From the charges already compiled"
      testID="generate-bill"
    >
      <VStack gap={14}>
        {!patientId ? (
          <Card>
            <VStack gap={10}>
              ...
              <SearchInput
                ...
                testID="generate-patient-search"
              />
          ...
          <>
              ...
              <HStack justify="space-between" align="center" wrap gap={8}>
                <VStack gap={2}>
                  ...
                  <Text variant="caption" tone="secondary">
                    {preview.patient.patientId} · {preview.patient.age} ·{" "}
                    {preview.patient.gender} · {preview.patient.mobile}
                  </Text>
                </VStack>
                {!route.params?.patientId ? (
                  <Button
            ...
            {generate.isError ? (
              <View testID="generate-error">
                <Banner
                  tone="danger"
                  title="No bill was generated"
                  message={apiErrorMessage(generate.error)}
                />
              </View>
            ) : null}
            ...
            {preview.drafts.length ? (
              <Banner
                ...
                message={preview.drafts
                  .map(
                    (d) =>
                      `${d.billNumber} (${d.billType === "ipd" ? "inpatient" : "outpatient"}, ${formatRupees(d.total)})`,
                  )
                  .join(", ")}
                action={
                  <Button
                    label={`Open ${preview.drafts[0].billNumber}`}
                    size="sm"
                    variant="secondary"
                    onPress={() =>
                      navigation.navigate("BillDetail", {
                        billId: preview.drafts[0].id,
                      })
                    }
                  />
                }
            ...
            <Card testID="preview-opd">
              <VStack gap={10}>
                <SectionHeader
                  title="Outpatient charges"
                  subtitle={`${preview.opd.lines.length} unbilled · ${formatRupees(preview.opd.total)}`}
                />
                ...
                <Button
                  ...
                  loading={
                    generate.isPending && generate.variables?.billType === "opd"
                  }
                  onPress={() => run("opd")}
                  testID="generate-opd"
                />
            ...
            {preview.admissions.map((a) => (
              <Card
                key={a.id}
                testID={`preview-admission-${a.admissionNumber}`}
              >
                <VStack gap={10}>
                  <SectionHeader
                  ...
                  {a.status === "admitted" ? (
                    <Text variant="caption" tone="secondary">
                      Still admitted — room charges are still accruing. The
                      final bill is generated after discharge.
                    </Text>
                  ) : (
                    <Button
                      ...
                      loading={
                        generate.isPending &&
                        generate.variables?.admissionId === a.id
                      }
