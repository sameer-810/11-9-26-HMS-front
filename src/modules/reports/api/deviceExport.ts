 ...
 */
export async function shareReportOnDevice(
  key: string,
  ...
  if (!FileSystem.cacheDirectory)
    throw new Error("This device has no space to save the export.");
  ...
  if (result.status !== 200) {
    ...
    try {
      message =
        JSON.parse(await FileSystem.readAsStringAsync(result.uri))?.error
          ?.message ?? message;
    } catch {
      /* not a JSON refusal */
    }
  ...
  await Sharing.shareAsync(result.uri, {
    mimeType: EXPORT_TYPES[format].mime,
