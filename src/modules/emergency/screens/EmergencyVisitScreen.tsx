  ...
  if (isError) {
    return (
      <Screen overline="Emergency" title="Attendance" testID="ed-visit">
        <ErrorState
          error={error}
          title="Couldn't load this attendance"
          onRetry={refetch}
        />
      </Screen>
    );
    ...
    >
      <VStack gap={14}>
        ...
        {banner?.accessRestricted ? (
          <Banner
            tone="warning"
            message="This patient's record is access-restricted. Opening the medical record is logged."
          />
        ) : null}
        ...
        {visit.esiLevel || !canTriage ? (
          <TriageSummary visit={visit} clinical={canTriage} />
        ) : null}
        ...
        {canTriage && active && arrived && !canTreat ? (
          <AssignCard visit={visit} />
        ) : null}

        {canTreat && active && arrived ? <DoctorCard visit={visit} /> : null}

        {canTreat && active && arrived ? (
          <DispositionCard visit={visit} onDone={setNotice} />
        ) : null}

        {canRunDepartment && active && !visit.seenByDoctorAt ? (
          <LeftCard visit={visit} onDone={setNotice} />
        ) : null}
    ...
    <Card testID="ed-visit-facts">
      <VStack gap={12}>
        ...
        <HStack gap={14} wrap>
          ...
          <Fact
            label="Arrived"
            value={v.arrivedAt ? formatDateTime(v.arrivedAt) : "Not yet"}
          />
          <Fact
            label="Expected"
            value={
              v.status === "expected" && v.expectedAt
                ? formatDateTime(v.expectedAt)
                : null
            }
          />
          <Fact
            label="Target to be seen"
            value={
              v.targetMinutes === null
                ? null
                : v.targetMinutes === 0
                  ? "Immediately"
                  : formatDuration(v.targetMinutes)
            }
          />
          ...
          <Fact
            label="Ambulance"
            value={[a?.service, a?.vehicleNumber, a?.crew]
              .filter(Boolean)
              .join(" · ")}
          />
          ...
          <Fact
            ...
            value={v.seenByDoctorAt ? formatDateTime(v.seenByDoctorAt) : null}
          />
    ...
    <Card>
      <VStack gap={10}>
        <Text variant="body-sm" tone="secondary">
          Ambulance pre-alert. Mark them arrived at the door — the department
          clock starts then.
        </Text>
        {arrive.isError ? (
          <Banner tone="danger" message={apiErrorMessage(arrive.error)} />
        ) : null}
        <Button
          label="Mark arrived"
          fullWidth={false}
          ...
          <>
            <HStack gap={10} align="center" wrap>
              ...
              <Text variant="caption" tone="tertiary">
                ...
                {clinical && v.triageCount
                  ? ` · triaged ${v.triageCount} time${v.triageCount === 1 ? "" : "s"}`
                  : ""}
              </Text>
            </HStack>
            {t?.overridden ? (
  ...
  return (
    <Card testID="ed-assign">
      <VStack gap={10}>
        <SectionHeader
          ...
          subtitle={
            visit.assignedDoctorName
              ? `Currently Dr ${visit.assignedDoctorName}`
              : undefined
          }
        />
        <Select
        ...
        />
        {assign.isError ? (
          <Banner tone="danger" message={apiErrorMessage(assign.error)} />
        ) : null}
        <Button
          label="Assign"
          fullWidth={false}
    ...
    <Card testID="ed-doctor">
      <VStack gap={10}>
        <SectionHeader
          ...
          subtitle={
            taken
              ? `With Dr ${visit.assignedDoctorName} since ${formatDateTime(visit.seenByDoctorAt)}`
              : "Taking the patient records the door-to-doctor time"
          }
        />
        {start.isError ? (
          <Banner tone="danger" message={apiErrorMessage(start.error)} />
        ) : null}
        <HStack gap={8} wrap>
          {!taken ? (
            <Button
            ...
            <Button
              ...
              icon={
                <ClipboardList size={15} color="#FFFFFF" strokeWidth={2.2} />
              }
        ...
        {taken && !visit.consultationId ? (
          <Text variant="caption" tone="tertiary">
            No consultation note was opened automatically. Start one from the
            medical record.
          </Text>
        ) : null}
      </VStack>
...
function DispositionCard({
  ...
  const noAdmission =
    dispose.isError && apiErrorCode(dispose.error) === "NO_ADMISSION";
  ...
  const submit = () =>
    ...
    dispose.mutate(
      ...
      {
        onSuccess: () =>
          onDone(`Disposition recorded: ${DISPOSITION_LABELS[type]}.`),
      },
    );
  ...
  return (
    <Card testID="ed-disposition">
      <VStack gap={10}>
        <SectionHeader
          title="Disposition"
          subtitle="Where the patient goes. This closes the attendance."
        />
        <ChoiceChips
        ...
        {type === "admitted" ? (
          <Text variant="caption" tone="tertiary">
            Admit the patient to a bed first. This records the admission already
            made.
          </Text>
        ) : null}
        <TextField
        ...
        {dispose.isError ? (
          <View testID="ed-dispose-error">
            <Banner
              ...
              title={
                noAdmission ? "No admission yet" : "Disposition not recorded"
              }
                    ...
                    onPress={() =>
                      navigation.navigate("AdmittedPatients", {
                        ...
                        params: {
                          ...
                          consultationId: visit.consultationId ?? undefined,
                        },
                      })
                    }
    ...
    <Card testID="ed-outcome">
      <VStack gap={6}>
        ...
        <Text variant="label-lg" tone="primary">
          {v.status === "left_without_being_seen"
            ? "Left without being seen"
            : d?.type
              ? DISPOSITION_LABELS[d.type]
              : ED_STATUS_LABELS[v.status]}
        </Text>
        {d ? (
          <Text variant="caption" tone="tertiary">
            {formatDateTime(d.at)}
