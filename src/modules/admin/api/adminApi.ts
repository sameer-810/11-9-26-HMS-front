...
export const adminApi = {
  ...
  users: {
    ...
    activate: async (id: string) =>
      (await apiClient.post<{ data: AdminUser }>(`/users/${id}/activate`)).data.data,
  ...
  hospital: {
    get: async () => (await apiClient.get<{ data: HospitalProfile }>("/hospital")).data.data,
    update: async (patch: HospitalPatch) =>
      (await apiClient.patch<{ data: HospitalProfile }>("/hospital", patch)).data.data,
  },

  // ---- Departments (write: hospital.config) --------------------------------
  departments: {
    list: async (params: { search?: string; isActive?: boolean; page?: number; limit?: number } = {}) =>
    ...
    activate: async (id: string) =>
      (await apiClient.post<{ data: Department }>(`/departments/${id}/activate`)).data.data,
  },

  // ---- Wards, rooms, beds (mounted at /beds) -------------------------------
  wards: {
    list: async () =>
      (await apiClient.get<Paged<Ward>>("/beds/wards", { params: { limit: MAX_LIMIT } })).data.data,
    create: async (body: WardBody) =>
      (await apiClient.post<{ data: Ward }>("/beds/wards", body)).data.data,
    update: async (id: string, body: Partial<WardBody> & { isActive?: boolean }) =>
      (await apiClient.patch<{ data: Ward }>(`/beds/wards/${id}`, body)).data.data,
  },

  rooms: {
    list: async (wardId: string) =>
      (await apiClient.get<Paged<Room>>("/beds/rooms", { params: { wardId, limit: MAX_LIMIT } })).data
        .data,
    create: async (body: { wardId: string; number: string; type?: RoomType; dailyCharge?: number }) =>
  ...
  beds: {
    ...
    ) => (await apiClient.patch<{ data: Bed }>(`/beds/${id}/status`, body)).data.data,
  },
};
