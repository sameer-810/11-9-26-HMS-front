...
export function StageStepper({ order }: { order: LabOrder }) {
  ...
  const current = LAB_STAGES.indexOf(
    order.status as (typeof LAB_STAGES)[number],
  );
  const eventFor = (stage: string) =>
    [...order.stageHistory].reverse().find((e) => e.to === stage);
      ...
      {LAB_STAGES.map((stage, i) => {
        ...
        return (
          <View
            key={stage}
            style={styles.step}
            accessibilityLabel={`${LAB_STAGE_LABELS[stage]}${done ? ", done" : active ? ", current" : ""}`}
          >
            <HStack gap={6} align="center">
              ...
              <VStack gap={0}>
                <Text
                  variant="label-sm"
                  tone={i <= current ? "primary" : "tertiary"}
                >
...
const styles = StyleSheet.create({
  ...
  dotDone: {
    backgroundColor: signal.normal.color,
    borderColor: signal.normal.color,
  },
  dotActive: {
