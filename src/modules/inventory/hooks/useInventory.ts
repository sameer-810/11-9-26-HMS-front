...
export const useUpdateItem = (id: string) => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (patch: Parameters<typeof inventoryApi.updateItem>[1]) =>
...
export const useReceiveStock = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: ReceiveBody) => inventoryApi.receive(body),
    onSuccess: () => invalidateStock(qc),
  });
};
...
export const useDisposeStock = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: inventoryApi.dispose,
    onSuccess: () => invalidateStock(qc),
  });
};
...
export const useStockMovements = (
  params: { itemId?: string; type?: MovementType; page?: number } = {},
) =>
  useQuery({
    queryKey: ["stock-movements", params],
    queryFn: () => inventoryApi.movements({ ...params, limit: 50 }),
  });
