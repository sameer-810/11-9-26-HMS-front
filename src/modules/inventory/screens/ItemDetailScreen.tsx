...
export default function ItemDetailScreen() {
  ...
  const batches = location
    ? data.batches.filter((b) => b.location === location)
    : data.batches;
  ...
  return (
    ...
    >
      <VStack gap={14}>
        {canEdit ? (
          <ItemSettings
            key={`${item.unitPrice}-${item.reorderLevel}`}
            itemId={item.id}
          ...
          <Card>
            <Text variant="body-sm" tone="secondary">
              ...
              per {item.unit} · reorder at {item.reorderLevel}
            </Text>
          </Card>
        )}
        ...
        <Card testID="item-batches">
          <SectionHeader
            title="Batches"
            subtitle="Soonest expiry first. Expired batches cannot be dispensed or issued."
          />
          {batches.length === 0 ? (
                ...
                <HStack gap={10} align="center" wrap>
                  ...
                  <VStack gap={1} style={{ flex: 1, minWidth: 220 }}>
                    <Text variant="body-sm">
                      {MOVEMENT_LABELS[m.type]} · {m.locationLabel} · batch{" "}
                      {m.batchNumber} · balance {m.balanceAfter}
                    </Text>
                    <Text variant="caption" tone="tertiary">
...
function ItemSettings({
  ...
  const save = () => {
    ...
    if (Number(reorder) !== reorderLevel)
      patch.reorderLevel = Math.max(0, Math.floor(Number(reorder) || 0));
    if (canPrice) {
      const next = price.trim() === "" ? null : Number(price);
      if (next !== unitPrice && (next === null || Number.isFinite(next)))
        patch.unitPrice = next;
    }
  ...
  return (
    <Card testID="item-settings">
      <VStack gap={10}>
        <HStack gap={10} wrap>
          ...
          {canPrice ? (
            ...
            <View style={{ flex: 1, minWidth: 160, justifyContent: "center" }}>
              <Text variant="caption" tone="tertiary">
                Price {unitPrice === null ? "not set" : formatRupees(unitPrice)}{" "}
        ...
        {update.isError ? (
          <Banner tone="danger" message={apiErrorMessage(update.error)} />
        ) : null}
        <Button
          label="Save"
          size="sm"
...
function BatchRow({
  ...
  const [reason, setReason] = useState(
    b.expiryStatus === "expired" ? "Expired on the shelf" : "",
  );
  ...
  return (
    <View
      style={styles.row}
      testID={`item-batch-${b.batchNumber}-${b.location}`}
    >
      <HStack gap={10} align="center" wrap>
        ...
        <VStack gap={8} style={{ marginTop: 8 }}>
          <HStack gap={10} wrap>
            ...
            <TextField
              ...
              testID={`dispose-reason-${b.batchNumber}`}
            />
          </HStack>
          {dispose.isError ? (
            <Banner tone="danger" message={apiErrorMessage(dispose.error)} />
          ) : null}
          <HStack gap={8}>
            <Button
