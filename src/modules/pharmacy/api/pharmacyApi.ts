...
import type { DispenseContext, Dispensing, PharmacyQueueRow } from "@modules/pharmacy/types";
...
export const pharmacyApi = {
  queue: async (params: { urgency?: string; search?: string } = {}) => {
    const res = await apiClient.get<{ data: PharmacyQueueRow[] }>("/pharmacy/queue", { params });
    return res.data.data;
  },

  context: async (prescriptionId: string) => {
    const res = await apiClient.get<{ data: DispenseContext }>(`/pharmacy/prescriptions/${prescriptionId}/dispense-context`);
    return res.data.data;
  },

  dispense: async (prescriptionId: string, body: DispenseBody) => {
    const res = await apiClient.post<{ data: { dispensing: Dispensing; prescription: Prescription } }>(
  ...
  dispensings: async (prescriptionId: string) => {
    const res = await apiClient.get<{ data: Dispensing[] }>(`/pharmacy/prescriptions/${prescriptionId}/dispensings`);
    return res.data.data;
  },
};
