...
await req(
  "POST",
  "/beds/bulk",
  { roomId: room.data.id, prefix: "A", from: 1, to: 2 },
  adminToken,
);
const beds = (
  await req("GET", `/beds?wardId=${ward.data.id}&limit=20`, null, adminToken)
).data;
...
const cbcTest = (
  await req("GET", "/laboratory/tests?search=blood", null, doctor.token)
).data.find((t) => t.code === "CBC");
const anitaCbc = (
  await req(
    "POST",
    "/laboratory/orders",
    {
      patientId: anita.id,
      testIds: [cbcTest.id],
      clinicalIndication: "Fever with raised respiratory rate",
      urgency: "stat",
    },
    doctor.token,
  )
...
const collected = (
  await req(
    "POST",
    `/laboratory/orders/${anitaCbc.id}/stage`,
    { to: "sample_collected" },
    lab.token,
  )
).data;
...
for (const to of ["sample_collected", "in_progress"])
  await req(
    "POST",
    `/laboratory/orders/${mohanCbc.id}/stage`,
    { to },
    lab.token,
  );
await req(
  "PUT",
  `/laboratory/orders/${mohanCbc.id}/results`,
  { values: { HB: "8.1", WBC: "7.0", PLT: "250", HCT: "30" } },
  lab.token,
);
...
const glycometItem = (
  await req(
    "POST",
    "/inventory/items",
    { code: "METF500", name: "METF500", medicineId: glycomet, unit: "tablet" },
    store.token,
  )
).data.id;
const supplier = (
  await req("POST", "/inventory/suppliers", { name: "MedLine" }, store.token)
).data.id;
await req(
  "POST",
  "/inventory/receipts",
...
console.log(
  `Seeded: ${anita.patientId} (sulfa, admitted ${admission.admissionNumber}, sample ${collected.sampleId}, ${rx.prescriptionNumber}) and ${mohan.patientId} (reported ${mohanCbc.orderNumber})\n`,
);
...
const web = http.createServer((rq, rs) => {
  ...
  if (url === "/__zxing/decode.html") {
    ...
    rs.end(
      '<!doctype html><html><body><script src="/__zxing/library.js"></script><script src="/__zxing/browser.js"></script></body></html>',
    );
    return;
  }
  if (ZXING[url]) {
...
async function renderAtSize(job, scale) {
  const ctx3 = await browser.newContext({
    viewport: {
      width: Math.ceil((job.widthMm * 96) / 25.4),
      height: Math.ceil((job.heightMm * 96) / 25.4),
    },
...
try {
  ...
  check(
    await ward1.getByTestId("print-wristband").isVisible(),
    "a nurse is offered Print wristband on the patient page",
  );
  await ward1.screenshot({
    path: path.join(SHOTS, "print-1-patient-detail.png"),
  });

  const band = await printJob(ward1, "print-wristband");
  check(
    band.widthMm === 25 && band.heightMm === 280,
    "the job asks for a 25 × 280 mm page",
    `${band.widthMm} × ${band.heightMm}`,
  );
  check(
    band.html.includes("@page { size: 25mm 280mm; margin: 0; }"),
    "and its document says so to the print pipeline",
  );
  ...
  check(
    ...
    size
      ? `${size.widthMm.toFixed(2)} × ${size.heightMm.toFixed(2)} mm`
      : "no MediaBox",
  );
  ...
  check(
    text === "ALLERGIES: Sulfa",
    "the allergy strip reads ALLERGIES: Sulfa",
    text,
  );
  text = await rendered.page.innerText("body");
  check(
    text.includes("CASE, Anita") &&
      text.includes(`Hosp no ${anita.patientId}`) &&
      text.includes("DOB 1 Jan 1970"),
    "surname first, hospital number and date of birth",
  );
  check(
    text.includes(admission.admissionNumber) &&
      text.includes("Medical Ward A") &&
      text.includes("Bed A1"),
    "admitted: admission number, ward and bed",
  );
  await rendered.page.screenshot({
    path: path.join(SHOTS, "print-2-wristband.png"),
    fullPage: true,
  });

  wristbandCode = await decode(
    await rendered.page.locator("#wristband-code128").screenshot(),
    { rotate: true },
  );
  check(
    wristbandCode === anita.patientId,
    "the band's Code 128, photographed at 2×, decodes to the hospital number",
    wristbandCode,
  );
  const bandPayload = await decode(
    await rendered.page.locator("[data-scan-payload] svg").screenshot(),
  );
  check(
    bandPayload === `HMS1|P|${anita.patientId}`,
    "its DataMatrix decodes to the versioned patient payload",
    bandPayload,
  );
  ...
  check(
    mohanBand.html.includes(
      'data-allergy-band="unrecorded">Allergies not recorded<',
    ),
    "a patient never asked prints Allergies not recorded, never a blank strip",
  );
  check(
    mohanBand.html.includes(mohan.patientId) &&
      !mohanBand.html.includes(anita.patientId),
    "and it is his band, not the last one printed",
  );
  ...
  const scanCode = async (p, code) => {
    ...
    check(
      ...
      `the scan field had focus and took "${code}"`,
    );
  ...
  check(
    ward1.url().includes(`/patients/${anita.id}/record`),
    "the decoded band opens Anita's record",
    ward1.url(),
  );
  check(
    (await body(ward1)).includes("Anita Case"),
    "with Anita's name on screen",
  );
  await ward1.screenshot({ path: path.join(SHOTS, "print-3-scan-record.png") });

  await scanCode(ward1, mohan.patientId);
  check(
    ward1.url().includes(`/patients/${mohan.id}/record`),
    "another patient's code opens the other patient",
    ward1.url(),
  );
  text = await body(ward1);
  check(
    text.includes("Mohan Das") && !text.includes("Anita Case"),
    "Mohan's name, and not Anita's",
  );

  await scanCode(ward1, `HMS1|P|${anita.patientId}`);
  check(
    ward1.url().includes(`/patients/${anita.id}/record`),
    "the 2D payload, typed by a 2D imager, opens Anita too",
  );

  await scanCode(ward1, "CGH-P999999");
  check(
    await ward1.getByTestId("scan-not-found").isVisible(),
    "a code that matches nobody says No patient matches this code",
  );
  check(
    new URL(ward1.url()).pathname === "/scan",
    "and goes nowhere",
    ward1.url(),
  );
  await ward1.screenshot({
    path: path.join(SHOTS, "print-4-scan-not-found.png"),
  });
  ...
  await bench.goto(`${WEB}/lab/requests/${anitaCbc.id}`, {
    waitUntil: "networkidle",
  });
  await bench.waitForTimeout(2600);
  check(
    await bench.getByTestId("print-tube-label").isVisible(),
    "a collected sample offers Print tube label",
  );
  const label = await printJob(bench, "print-tube-label");
  check(
    label.widthMm === 50 && label.heightMm === 25,
    "the job asks for a 50 × 25 mm label",
  );
  const labelSize = await pdfPageSize(label.html);
  check(
    labelSize &&
      within(labelSize.widthMm, 50) &&
      within(labelSize.heightMm, 25),
    "Chromium's PDF of it is 50 × 25 mm (±0.5)",
    labelSize
      ? `${labelSize.widthMm.toFixed(2)} × ${labelSize.heightMm.toFixed(2)} mm`
      : "no MediaBox",
  );
  check(
    label.html.includes('data-urgency="stat">STAT<'),
    "the STAT order is marked STAT",
  );
  check(
    label.html.includes("CASE, Anita") &&
      label.html.includes(`${anita.patientId} · DOB 1 Jan 1970`),
    "name, hospital number and date of birth",
  );

  const labelPage = await renderAtSize(label, 2);
  await labelPage.page.screenshot({
    path: path.join(SHOTS, "print-5-tube-label.png"),
  });
  const tubeCode = await decode(
    await labelPage.page.locator("#specimen-code128").screenshot(),
  );
  check(
    tubeCode === collected.sampleId,
    "its Code 128 decodes to the sample number",
    tubeCode,
  );
  const specimenPayload =
    /data-scan-payload="([^"]+)"/.exec(label.html)?.[1] ?? "";
  const tubePayload = await decode(
    await labelPage.page.locator("[data-scan-payload] svg").screenshot(),
  );
  check(
    tubePayload === specimenPayload &&
      specimenPayload === `HMS1|S|${collected.sampleId}`,
    "its DataMatrix decodes to the specimen payload",
    tubePayload,
  );
  ...
  check(
    bench.url().includes(`/lab/requests/${anitaCbc.id}`),
    "the label's payload, scanned, opens that laboratory order",
    bench.url(),
  );
  text = await body(bench);
  check(
    text.includes(collected.sampleId) && text.includes("Anita Case"),
    "with the sample number and the patient on screen",
  );

  // =========================================================================
  console.log("\nPR-04 — prescription and report\n");

  await bench.goto(`${WEB}/lab/requests/${mohanCbc.id}`, {
    waitUntil: "networkidle",
  });
  await bench.waitForTimeout(2600);
  const report = await printJob(bench, "print-lab-report");
  check(
    report.html.includes("Mohan Das") && report.html.includes(mohan.patientId),
    "the lab report is Mohan's",
  );
  check(!report.html.includes("Anita"), "and nobody else's");
  check(
    report.html.includes("Suspected anaemia") &&
      report.html.includes("Haemoglobin"),
    "with the indication and the reported values",
  );
  const reportSize = await pdfPageSize(report.html);
  check(
    reportSize &&
      within(reportSize.widthMm, 210) &&
      within(reportSize.heightMm, 297),
    "on A4",
  );
  const reportPage = await renderAtSize(report, 1);
  await reportPage.page.screenshot({
    path: path.join(SHOTS, "print-7-lab-report.png"),
    fullPage: true,
  });
  ...
  await counter.goto(`${WEB}/pharmacy/dispense/${rx.id}`, {
    waitUntil: "networkidle",
  });
  await counter.waitForTimeout(2800);
  const script = await printJob(counter, "print-prescription");
  check(
    script.html.includes("Anita Case") && script.html.includes(anita.patientId),
    "the prescription is Anita's",
  );
  check(
    script.html.includes("Glycomet") &&
      script.html.includes("ALLERGIES: Sulfa"),
    "with the medicine and her allergy",
  );
  check(script.html.includes("Dr Rajesh Rao"), "and the prescriber");
  const scriptSize = await pdfPageSize(script.html);
  check(
    scriptSize &&
      within(scriptSize.widthMm, 148) &&
      within(scriptSize.heightMm, 210),
    "on A5",
  );
  const scriptPage = await renderAtSize(script, 1);
  await scriptPage.page.screenshot({
    path: path.join(SHOTS, "print-6-prescription.png"),
    fullPage: true,
  });
  ...
  const unexpected = httpFailures.filter(
    (f) => f !== "404 GET /api/v1/patients/scan/CGH-P999999",
  );
  check(
    unexpected.length === 0,
  ...
  await ward1
    .screenshot({ path: path.join(SHOTS, "print-FAILURE.png") })
    .catch(() => {});
} finally {
  await browser.close();
  web.close();
