...
const ExcelJS = (
  await imp(BACK, "node_modules", "exceljs", "lib", "exceljs.nodejs.js")
).default;
...
await req(
  "POST",
  "/beds/bulk",
  { roomId, prefix: "A", from: 1, to: 3 },
  adminToken,
);
const beds = (
  await req("GET", `/beds?wardId=${wardId}&limit=10`, null, adminToken)
).data;
await req(
  "POST",
  "/admissions",
...
await req(
  ...
  { patientId: dev.id, doctorId: doctor.id, reason: "Fever since morning" },
  reception.token,
);
...
async function recommend(patientId, reason) {
  ...
  await req(
    ...
    `/consultations/${consult.id}`,
    {
      ...
      diagnoses: [
        { description: "Heart failure", type: "provisional", isPrimary: true },
      ],
...
const chitraRequest = await recommend(
  chitra.id,
  "Needs IV diuretics and monitoring",
);
const bhavnaRequest = await recommend(bhavna.id, "Observation overnight");

console.log(
  "Seeded: an admission, a walk-in and two recommendations to admit\n",
);
...
async function download(p, testId) {
  ...
  const saved = path.join(
    os.tmpdir(),
    `hms-verify-${Date.now()}-${file.suggestedFilename()}`,
  );
...
try {
  // =========================================================================
  console.log(
    "US-01 / US-05 — reception signs in with an employee ID; the cards open their lists\n",
  );

  await signIn(desk, "rec001", reception.password);
  check(
    await desk.getByTestId("dashboard").isVisible(),
    "the employee ID on the badge signs in, whatever its case",
  );
  text = await desk.getByTestId("tile-patients").innerText();
  check(
    /Total patients\s*4/.test(text),
    "the dashboard counts total patients",
    text.replace(/\s+/g, " "),
  );
  text = await desk.getByTestId("tile-opd").innerText();
  check(
    /OPD patients now\s*1/.test(text) && text.includes("1 waiting"),
    "and the OPD patients waiting now",
    text.replace(/\s+/g, " "),
  );
  check(
    (await desk.getByTestId("tile-appointments").innerText()).includes(
      "Today's appointments",
    ),
    "and today's appointments",
  );
  check(
    (await desk.getByTestId("tile-inpatients").count()) === 0,
    "but no inpatient figures for reception",
  );
  ...
  check(
    pathOf(desk).endsWith("/opd/queue"),
    "the OPD card opens the OPD queue",
    pathOf(desk),
  );
  ...
  check(
    /\/patients\/?$/.test(pathOf(desk)) ||
      (await desk.getByTestId("patients-screen").count()) > 0,
    "the patients card opens patient search",
    pathOf(desk),
  );
  ...
  check(
    (await clinic.getByTestId("tile-appointments").innerText()).includes(
      "My appointments today",
    ),
    "a doctor's appointment card is their own day",
  );
  text = await clinic.getByTestId("tile-admission-requests").innerText();
  check(
    /Waiting for a bed\s*2/.test(text),
    "two recommendations wait for a bed",
    text.replace(/\s+/g, " "),
  );
  await clinic.getByTestId("tile-admission-requests").click();
  await clinic.waitForTimeout(2600);
  check(
    pathOf(clinic).endsWith("/ipd/patients"),
    "the card opens the admitted patients board",
    pathOf(clinic),
  );
  text = await clinic.getByTestId("admission-requests").innerText();
  check(
    text.includes("2 waiting for a bed") &&
      text.includes("Needs IV diuretics and monitoring") &&
      text.includes("Heart failure"),
    "the board lists them with the doctor's reason and diagnosis",
  );
  ...
  check(
    await clinic.getByTestId("request-close-submit").isDisabled(),
    "a recommendation cannot be closed without a note",
  );
  await clinic
    .getByTestId("request-close-note")
    .fill("Chose to go home, will return if worse");
  await clinic.waitForTimeout(300);
  await clinic.getByTestId("request-close-submit").click();
  await clinic.waitForTimeout(2600);
  text = await clinic.getByTestId("admission-requests").innerText();
  check(
    text.includes("1 waiting for a bed") &&
      !text.includes("Observation overnight"),
    "closed with a reason, it leaves the list",
  );

  await clinic.getByTestId(`request-admit-${chitraRequest}`).click();
  await clinic.waitForTimeout(2400);
  check(
    pathOf(clinic).endsWith("/ipd/admit"),
    "Admit opens the admission form",
    pathOf(clinic),
  );
  check(
    (await clinic.getByTestId("chosen-patient").innerText()).includes(
      "Chitra Case",
    ),
    "with the patient chosen",
  );
  check(
    (await clinic.getByTestId("admit-reason").inputValue()) ===
      "Needs IV diuretics and monitoring",
    "and the doctor's reason filled in",
  );
  ...
  check(
    pathOf(office).endsWith("/reports"),
    "administration's patients card opens the registrations report, not a patient list",
    pathOf(office),
  );
  check(
    /Registered\s*4/.test(
      await office.getByTestId("report-summary").innerText(),
    ),
    "which counts the four registrations",
  );
  ...
  check(
    /Consultations signed\s*2/.test(text),
    "doctor activity counts the consultations signed",
    text.replace(/\s+/g, " ").slice(0, 160),
  );
  text = await office.getByTestId("report-table").innerText();
  check(
    text.includes("Rajesh Rao") && text.includes("Farah Rao"),
    "per doctor, idle doctors included",
  );
  for (const name of ["Anita", "Bhavna", "Chitra"])
    check(
      !(await body(office)).includes(`${name} Case`),
      `no patient named in the report (${name})`,
    );
  await office.screenshot({
    path: path.join(SHOTS, "stories-3-doctor-activity.png"),
  });

  const xlsx = await download(office, "report-export-xlsx");
  check(
    /^doctor_activity_\d{4}-\d{2}-\d{2}_\d{4}-\d{2}-\d{2}\.xlsx$/.test(
      xlsx.name,
    ),
    "the report downloads as an Excel file",
    xlsx.name || "no download",
  );
  let sheetText = "";
  try {
    ...
    sheetText = workbook
      ...
      .join(" ");
    check(
      workbook.getWorksheet("Table").getRow(2).getCell(1).value ===
        "Rajesh Rao",
      "a real workbook, with the table in it",
    );
  } catch (err) {
    check(false, "a real workbook, with the table in it", err.message);
  }
  check(
    sheetText.includes("Doctor activity") &&
      sheetText.includes("City General Hospital"),
    "headed with the report and the hospital",
  );

  const pdf = await download(office, "report-export-pdf");
  check(
    /^doctor_activity_.*\.pdf$/.test(pdf.name) &&
      pdf.bytes.subarray(0, 5).toString("latin1") === "%PDF-",
    "and as a PDF",
    pdf.name || "no download",
  );
  ...
  check(
    await office.getByTestId("roles-screen").isVisible(),
    "the roles screen opens from its own address",
  );
  const locked = office.getByTestId("role-permission-record.view");
  check(
    (await locked.getAttribute("aria-disabled")) === "true",
    "reception cannot be given the medical record",
  );
  check(
    (await locked.innerText()).includes("Not in this role's standard set"),
  ...
  check(
    (await req("GET", "/beds/board", null, billing.token)).status === 403,
    "billing staff start without the bed board",
  );
  await office.getByTestId("role-permission-beds.view").click();
  await office.waitForTimeout(300);
  check(
    (await office.getByTestId("role-diff").innerText()).includes("Adding"),
    "the change is spelled out before it is saved",
  );
  ...
  check(
    text.includes("Applied to the 1 person"),
    "saved and applied to the one person in the role",
    text,
  );
  check(
    (await req("GET", "/beds/board", null, billing.token)).status === 200,
    "who can open the bed board from their next request",
  );
  check(
    (await office.getByTestId("role-origin").innerText()).includes(
      "Changed for this hospital",
    ),
    "the role is marked as changed",
  );
  ...
  check(
    ...
    "and resets to the standard set",
  );
  ...
  check(
    await office.getByTestId("user-wards-saved").isVisible(),
    "administration allocates the nurse to Medical Ward A",
  );
  ...
  check(
    /My patients\s*1/.test(text),
    "the nurse's dashboard counts the ward's patient",
    text.replace(/\s+/g, " "),
  );
  await ward.getByTestId("tile-my-patients").click();
  await ward.waitForTimeout(2600);
  check(
    (await body(ward)).includes("Anita Case"),
    "and the ward list shows her, though nobody allocated her by name",
  );
  ...
  check(
    (await body(office)).includes("Between 5 and 480 minutes"),
    "a timeout under five minutes is refused on the form",
  );
  await office.getByTestId("hospital-sessionIdleMinutes").fill("5");
  await office.getByTestId("hospital-save").click();
  await office.waitForTimeout(2400);
  check(
    await office.getByTestId("hospital-saved").isVisible(),
    "five minutes is saved",
  );
  ...
  check(
    await idle.getByTestId("dashboard").isVisible(),
    "a nurse signs in on a shared computer",
  );
  await idle.clock.fastForward("04:05");
  await idle.waitForTimeout(1500);
  check(
    await idle.getByTestId("idle-warning").isVisible(),
    "four minutes untouched, the screen warns",
  );
  check(
    (await idle.getByTestId("idle-warning").innerText()).includes(
      "signs out in",
    ),
    "and counts down",
  );
  await idle.screenshot({
    path: path.join(SHOTS, "stories-6-idle-warning.png"),
  });
  await idle.getByTestId("idle-stay").click();
  await idle.waitForTimeout(800);
  check(
    (await idle.getByTestId("idle-warning").count()) === 0,
    "Stay signed in keeps the session",
  );
  check(
    await idle.getByTestId("dashboard").isVisible(),
    "on the screen the nurse was using",
  );

  await idle.clock.fastForward("05:02");
  await idle.waitForTimeout(2500);
  check(
    await idle.getByTestId("login-submit").isVisible(),
    "left alone past the timeout, the screen signs out",
  );
  text = await idle.getByTestId("login-session-notice").innerText();
  check(
    text.includes("5 minutes without activity"),
    "and the sign-in screen says why",
    text,
  );
  check(
    !(await body(idle)).includes("Anita Case"),
    "with nothing of the last session left on screen",
  );
  ...
  await current
    .screenshot({ path: path.join(SHOTS, "stories-FAILURE.png") })
    .catch(() => {});
} finally {
  await browser.close();
  web.close();
