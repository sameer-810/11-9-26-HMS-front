...
const { sweep } = await imp(BACK, "src", "modules", "laboratory", "labEscalation.js");

const hospital = await HospitalModel.create({
  name: "City General Hospital", code: "CGH",
  approvalStatus: "approved", approvedAt: new Date(), isActive: true, timezone: "Asia/Kolkata",
});
...
const adminToken = (
  await req("POST", "/auth/login", {
    email: "admin@cgh.test", password: "AdminPassword123", deviceId: "verify-admin-device", deviceName: "Verifier",
  })
).data.accessToken;
...
const colleague = await provision({ employeeId: "DOC002", firstName: "Farah", email: "farah@cgh.test", role: "doctor", departmentId: dept.data.id });
...
const patient = await req("POST", "/patients", {
  firstName: "Mohan", lastName: "Das", gender: "male", dateOfBirth: "1966-03-02", mobile: "9876543210",
}, reception.token);
const patientId = patient.data.id;
await req("PUT", `/patients/${patientId}/allergies`, { allergies: [] }, doctor.token);

await req("POST", "/laboratory/tests/load-standard", null, adminToken);
const catalogue = (await req("GET", "/laboratory/tests", null, doctor.token)).data;
...
const consultation = await req("POST", "/consultations", { patientId, type: "opd" }, doctor.token);
await req("PATCH", `/consultations/${consultation.data.id}`, {
  chiefComplaint: "Breathless and weak for three days",
  diagnoses: [{ description: "Chronic kidney disease stage 4", type: "provisional", isPrimary: true }],
  treatmentPlan: "Restrict potassium, review dialysis referral",
}, doctor.token);
await req("POST", `/consultations/${consultation.data.id}/sign`, null, doctor.token);

// A routine test ordered earlier — the stat order placed later must overtake it.
const tsh = (await req("POST", "/laboratory/orders", {
  patientId, testIds: [byCode.TSH.id], clinicalIndication: "Fatigue and weight gain", urgency: "routine",
}, doctor.token)).data.orders[0];

console.log("Seeded: a CKD patient, a signed diagnosis, the standard catalogue and a routine TSH\n");
...
try {
  ...
  await doc.goto(`${WEB}/patients/${patientId}/record`, { waitUntil: "networkidle" });
  await doc.waitForTimeout(2400);
  await doc.getByText("Lab results", { exact: false }).first().click();
  await doc.waitForTimeout(1200);

  check(await doc.getByTestId("order-tests-panel").isVisible(), "the order panel is on the patient's record");
  check(!(await body(doc)).includes("Patient name"), "there is no patient field to type into");
  ...
  check(text.includes("Fasting for 8 to 10 hours"), "preparation instructions appear as the test is chosen");
  check(!/₹|\bprice\b/i.test(text), "no price anywhere in the doctor's ordering panel");
  check(await doc.getByTestId("lab-order-submit").isDisabled(), "an order without a clinical indication cannot be sent");

  await doc.getByTestId("lab-urgency-stat").click();
  await doc.getByTestId("lab-indication").fill("CKD with weakness and breathlessness — check potassium and haemoglobin");
  ...
  check(text.includes("Waiting for the laboratory") && text.includes("Renal function test"), "and listed as waiting on the record");
  ...
  check(await doc.getByTestId("lab-duplicate-warning").isVisible(), "LB-01: ordering the same test again pauses with the earlier order");
  check((await body(doc)).includes("ordered") && (await body(doc)).includes("by Rajesh"), "naming who ordered it and when");
  await doc.screenshot({ path: path.join(SHOTS, "lab-2-duplicate.png") });
  await doc.getByText("Don't order").click();
  await doc.waitForTimeout(500);

  const inbox = (await req("GET", "/laboratory/inbox", null, doctor.token)).data;
  rftOrder = inbox.pending.find((o) => o.test.code === "RFT");
  const cbcOrder = inbox.pending.find((o) => o.test.code === "CBC");
  check(inbox.pending.filter((o) => o.test.code === "RFT").length === 1, "only one renal panel was actually ordered");
  ...
  const rows = await bench.locator('[data-testid^="lab-row-"]').evaluateAll((els) => els.map((e) => e.getAttribute("data-testid")));
  const idx = (n) => rows.indexOf(`lab-row-${n}`);
  check(idx(rftOrder.orderNumber) >= 0 && idx(tsh.orderNumber) >= 0, "the stat and routine orders are both in the queue");
  check(idx(rftOrder.orderNumber) < idx(tsh.orderNumber), "LB-02: the newer STAT order sorts above the older routine one");
  ...
  check(text.includes("check potassium and haemoglobin"), "Flow 3 step 3: the clinical indication is on the bench");
  ...
  check(/SMP-\d{6}/.test(await bench.getByTestId("lab-sample-id").innerText()), "collecting the sample issues its accession number");
  await bench.getByTestId("advance-in_progress").click();
  await bench.waitForTimeout(1800);

  text = await body(bench);
  check(text.includes("3.5 – 5.1 mmol/L"), "LB-04: the potassium field shows its range");
  check(text.includes("0.7 – 1.3 mg/dL (male"), "and the creatinine range is the one for this 60-year-old man");
  ...
  check((await bench.getByTestId("param-K-preview").innerText()).includes("HH"), "a potassium of 6.9 turns critical as it is typed, before saving");
  check((await body(bench)).includes("Critically high"), "and says so in words");
  check((await body(bench)).includes("Not a number"), "\"13,6\" is refused on the spot");
  check(await bench.getByTestId("save-results").isDisabled(), "and nothing can be saved while it is there");
  await bench.screenshot({ path: path.join(SHOTS, "lab-4-entry-live-flag.png") });
  ...
  await bench.waitForTimeout(2200);
  check((await body(bench)).includes("Critical: Potassium"), "saving confirms the critical value from the server");

  // ---- The doctor cannot read bench values ---------------------------------
  await doc.goto(`${WEB}/lab/reports/${rftOrder.id}`, { waitUntil: "networkidle" });
  await doc.waitForTimeout(2400);
  text = await body(doc);
  check(await doc.getByTestId("results-not-reported").isVisible(), "a doctor opening an unreported order is told results are not reported yet");
  ...
  check(await bench.getByTestId("critical-unreported").isVisible(), "a completed critical result warns that only the lab knows about it");

  await bench.getByTestId("advance-reported").click();
  await bench.waitForTimeout(900);
  check((await body(bench)).includes("Report a critical result?"), "reporting a critical value is confirmed, naming who will be alerted");
  await bench.getByText("Report and alert").click();
  await bench.waitForTimeout(2600);

  text = await body(bench);
  check(text.includes("Critical result — not yet acknowledged"), "LB-05: the critical result is open once reported");
  check(text.includes("ordering doctor"), "and the ordering doctor is recorded as notified");
  check((await bench.getByTestId("critical-acknowledge-form").count()) === 0, "the laboratory is not offered a way to close its own critical call");
  ...
  await bench.waitForTimeout(2000);
  check((await body(bench)).includes("told Dr Rajesh, read back"), "the lab records the call and the read-back");
  await bench.screenshot({ path: path.join(SHOTS, "lab-5-reported-critical.png") });

  // ---- Sample rejection on the CBC -----------------------------------------
  await bench.goto(`${WEB}/lab/requests/${cbcOrder.id}`, { waitUntil: "networkidle" });
  await bench.waitForTimeout(2200);
  await bench.getByTestId("advance-sample_collected").click();
  ...
  await bench.waitForTimeout(2000);
  check((await bench.getByTestId("sample-rejections").innerText()).includes("clotted"), "LB-03: a rejected sample keeps its reason on the order");
  check(await bench.getByTestId("advance-sample_collected").isVisible(), "and returns to collection, the one sanctioned step back");
  ...
  await doc.waitForTimeout(2600);
  text = await body(doc);
  check(await doc.getByTestId("inbox-critical").isVisible(), "the critical result is at the top of the doctor's results");
  ...
  check(escalated.length === 1, "unacknowledged after the window, it escalates");
  ...
  check(text.includes("Mohan Das") && text.includes("Potassium 6.9"), "a colleague in the department now sees a result she did not order");
  check(text.includes("1 doctor in the department"), "and can see how far it has escalated");
  ...
  check(await farahPage.getByTestId("critical-acknowledge-submit").isDisabled(), 'LB-05: "seen" cannot acknowledge a critical result');

  await farahPage.getByTestId("critical-acknowledge-note").fill(
  ...
  await farahPage.waitForTimeout(2400);
  text = await body(farahPage);
  check(text.includes("Acknowledged by Farah"), "the colleague acknowledges it with what she is doing");
  await farahPage.screenshot({ path: path.join(SHOTS, "lab-7-acknowledged.png") });

  await doc.goto(`${WEB}/lab/reports`, { waitUntil: "networkidle" });
  await doc.waitForTimeout(2400);
  check((await doc.getByTestId("inbox-critical").count()) === 0, "and it leaves the ordering doctor's critical list");
  ...
  await doc.getByText("Lab results", { exact: false }).first().click();
  await doc.waitForTimeout(1200);
  check(await doc.getByTestId(`record-lab-${rftOrder.orderNumber}`).isVisible(), "the reported result is on the patient's record");
  check((await doc.getByTestId("result-K-flag").first().innerText()).includes("HH"), "flagged HH beside the value");
  check((await body(doc)).includes("acknowledged by Farah"), "with who acknowledged the critical value");
  await doc.screenshot({ path: path.join(SHOTS, "lab-8-record.png") });

  await bench.goto(`${WEB}/patients/${patientId}/record`, { waitUntil: "networkidle" });
  await bench.waitForTimeout(2400);
  text = await body(bench);
  check(text.includes("Laboratory view"), "MR-03: the laboratory is told which view of the record it has");
  check(!text.includes("Chronic kidney disease"), "section 6.4: and it does not contain the diagnosis");
  check(!text.includes("dialysis referral"), "or the treatment plan");

  await bench.goto(`${WEB}/dashboard`, { waitUntil: "networkidle" });
  await bench.waitForTimeout(2200);
  check((await body(bench)).includes("Tests pending"), "the laboratory's dashboard shows pending tests");
  ...
  const unexpected = httpFailures.filter((f) => !/^409 POST \/api\/v1\/laboratory\/orders$/.test(f));
  check(unexpected.length === 0, "no unexpected HTTP failures", unexpected.join(", "));
} catch (err) {
  console.error("\nJourney threw:", err.message);
  failures.push(`exception: ${err.message}`);
  await doc.screenshot({ path: path.join(SHOTS, "lab-FAILURE.png") }).catch(() => {});
} finally {
  await browser.close();
  web.close();
  api.kill("SIGTERM");
