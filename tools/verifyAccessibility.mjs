...
].filter((v) => !process.env.A11Y_VIEWPORTS || process.env.A11Y_VIEWPORTS.split(",").includes(v.name));
...
const must = async (label, promise) => {
  ...
  if (!r || r.success === false) seedProblems.push(`${label}: ${r?.message ?? "no response"}`);
  return r?.data;
};
...
const med = await must("department MED", req("POST", "/departments", { name: "General Medicine", code: "MED" }, adminToken));
const ortho = await must("department ORT", req("POST", "/departments", { name: "Orthopaedics", code: "ORT" }, adminToken));
await DepartmentModel.updateOne({ _id: med.id }, { consultationFee: 500 });

async function provision({ employeeId, firstName, email, role, departmentId, keepTemporary = false }) {
...
const users = {
  receptionist: await provision({ employeeId: "REC001", firstName: "Deepak", email: "deepak@cgh.test", role: "receptionist" }),
  doctor: await provision({ employeeId: "DOC001", firstName: "Rajesh", email: "rajesh@cgh.test", role: "doctor", departmentId: med.id }),
  nurse: await provision({ employeeId: "NUR001", firstName: "Lakshmi", email: "lakshmi@cgh.test", role: "nurse" }),
  lab: await provision({ employeeId: "LAB001", firstName: "Sunita", email: "sunita@cgh.test", role: "lab" }),
  pharmacy: await provision({ employeeId: "PHA001", firstName: "Imran", email: "imran@cgh.test", role: "pharmacy" }),
  billing: await provision({ employeeId: "BIL001", firstName: "Priya", email: "priya@cgh.test", role: "billing" }),
  inventory: await provision({ employeeId: "INV001", firstName: "Kavya", email: "kavya@cgh.test", role: "inventory" }),
  admin: { email: "admin@cgh.test", password: "AdminPassword123", token: adminToken },
};
const { receptionist: reception, doctor, nurse, lab, pharmacy: pharmacist, billing, inventory: store } = users;
...
const newStarter = await provision({ employeeId: "NUR009", firstName: "Meera", email: "meera@cgh.test", role: "nurse", keepTemporary: true });

const register = async (firstName, lastName, gender, dateOfBirth, mobile) =>
  must(`patient ${firstName}`, req("POST", "/patients", { firstName, lastName, gender, dateOfBirth, mobile }, reception.token));
const anita = await register("Anita", "Case", "female", "1970-01-01", "9876500001");
const sanjay = await register("Sanjay", "Case", "male", "1960-01-01", "9876500002");
const mohan = await register("Mohan", "Das", "male", "1966-03-02", "9876500003");
const bhavna = await register("Bhavna", "Case", "female", "1985-05-05", "9876500004");
const kiran = await register("Kiran", "Case", "male", "1990-07-07", "9876500005");
const rina = await register("Rina", "Case", "female", "1982-02-02", "9876500006");
...
await must("roster", req("POST", "/appointments/roster", { doctorId: doctor.id, departmentId: med.id, dayOfWeek: clinic.getDay(), startTime: "10:00", endTime: "13:00", slotMinutes: 15, slotCapacity: 1 }, adminToken));
await must("appointment", req("POST", "/appointments", { patientId: bhavna.id, doctorId: doctor.id, departmentId: med.id, date: clinicDate, time: "10:30", visitType: "new", reason: "Follow-up of blood pressure" }, reception.token));

// Medicines and stock, including a low-stock line.
const medicine = async (name, ingredient) => must(`medicine ${name}`, req("POST", "/prescriptions/medicines", { name, genericName: ingredient, ingredients: [ingredient], form: "tablet", strength: "500mg" }, adminToken));
const glycomet = (await medicine("Glycomet", "metformin")).id;
const calcirol = (await medicine("Calcirol", "cholecalciferol")).id;
const item = async (code, body) => (await must(`item ${code}`, req("POST", "/inventory/items", { code, name: code, unit: "tablet", ...body }, store.token))).id;
...
await must("price METF500", req("PATCH", `/inventory/items/${glycometItem}`, { unitPrice: 3 }, adminToken));
await must("price VITD", req("PATCH", `/inventory/items/${calcirolItem}`, { unitPrice: 5 }, adminToken));
const supplier = (await must("supplier", req("POST", "/inventory/suppliers", { name: "MedLine" }, store.token))).id;
await must("pharmacy receipt", req("POST", "/inventory/receipts", {
...
await must("store receipt (low stock)", req("POST", "/inventory/receipts", {
  location: "main_store", supplierId: supplier, invoiceNumber: "INV-2",
...
const rxPending = (await must("prescription (pending)", req("POST", "/prescriptions", {
  patientId: anita.id, consultationId: consult.id,
  lines: [{ medicineId: glycomet, dose: "1 tab", frequency: "bd", durationDays: 5 }],
}, doctor.token)))?.prescription;
await must("sign consultation", req("POST", `/consultations/${consult.id}/sign`, null, doctor.token));
const rxDispensed = (await must("prescription (to dispense)", req("POST", "/prescriptions", {
  patientId: anita.id, lines: [{ medicineId: calcirol, dose: "1 tab", frequency: "od", durationDays: 4 }],
}, doctor.token)))?.prescription;
if (rxDispensed) {
  const ctx = await must("dispense context", req("GET", `/pharmacy/prescriptions/${rxDispensed.id}/dispense-context`, null, pharmacist.token));
  if (ctx) {
    await must("dispense", req("POST", `/pharmacy/prescriptions/${rxDispensed.id}/dispense`, {
...
const catalogue = (await must("lab tests", req("GET", "/laboratory/tests", null, doctor.token))) ?? [];
const testId = (code) => catalogue.find((t) => t.code === code)?.id;
const cbc = (await must("CBC order", req("POST", "/laboratory/orders", { patientId: anita.id, testIds: [testId("CBC")], clinicalIndication: "Suspected anaemia", urgency: "routine" }, doctor.token)))?.orders?.[0];
const report = async (order, values) => {
  for (const to of ["sample_collected", "in_progress"]) await must(`stage ${to}`, req("POST", `/laboratory/orders/${order.id}/stage`, { to }, lab.token));
  await must("results", req("PUT", `/laboratory/orders/${order.id}/results`, { values }, lab.token));
  for (const to of ["completed", "reported"]) await must(`stage ${to}`, req("POST", `/laboratory/orders/${order.id}/stage`, { to }, lab.token));
};
if (cbc) await report(cbc, { HB: "8.1", WBC: "7.0", PLT: "250", HCT: "30" });
// Mohan: a critical potassium.
const rft = (await must("RFT order", req("POST", "/laboratory/orders", { patientId: mohan.id, testIds: [testId("RFT")], clinicalIndication: "Weakness", urgency: "stat" }, doctor.token)))?.orders?.[0];
if (rft) await report(rft, { UREA: "40", CREAT: "1.1", NA: "138", K: "7.1", CL: "101" });

// Sanjay: admitted, observations, and an escalation raised by the score.
const wardId = (await must("ward", req("POST", "/beds/wards", { name: "Medical Ward A", code: "MWA", type: "general", departmentId: med.id }, adminToken))).id;
const roomId = (await must("room", req("POST", "/beds/rooms", { wardId, number: "101", type: "general" }, adminToken))).id;
await must("beds", req("POST", "/beds/bulk", { roomId, prefix: "A", from: 1, to: 4 }, adminToken));
const beds = (await must("bed list", req("GET", `/beds?wardId=${wardId}&limit=20`, null, adminToken))) ?? [];
const admission = await must("admission", req("POST", "/admissions", { patientId: sanjay.id, bedId: beds[0]?.id, reason: "Community acquired pneumonia, IV antibiotics" }, doctor.token));
await must("assign nurse", req("POST", `/admissions/${admission.id}/nurse`, { nurseId: nurse.id }, doctor.token));
await must("observations (calm)", req("POST", "/nursing/observations", { admissionId: admission.id, respiratoryRate: 16, spo2: 97, onOxygen: false, systolic: 124, pulse: 78, consciousness: "alert", temperatureC: 37.1 }, nurse.token));
await must("observations (escalating)", req("POST", "/nursing/observations", { admissionId: admission.id, respiratoryRate: 26, spo2: 91, onOxygen: false, systolic: 95, pulse: 122, consciousness: "alert", temperatureC: 38.4 }, nurse.token));

// Emergency: one triaged, one waiting for triage.
const edVisit = await must("ED visit", req("POST", "/emergency/visits", { patientId: bhavna.id, arrivalMode: "walk_in", chiefComplaint: "Sprained ankle" }, reception.token));
await must("triage", req("POST", `/emergency/visits/${edVisit.id}/triage`, { answers: { expectedResources: [] }, vitals: {} }, nurse.token));
await must("ED visit (untriaged)", req("POST", "/emergency/visits", { patientId: kiran.id, arrivalMode: "ambulance", chiefComplaint: "Chest pain" }, reception.token));

// Rina: restricted, and a doctor outside the team breaks the glass.
await must("Rina consultation", req("POST", "/consultations", { patientId: rina.id, type: "opd" }, doctor.token));
await must("restriction", req("POST", `/access/patients/${rina.id}/restriction`, { restricted: true, reason: "Staff member, requested confidentiality" }, doctor.token));
await must("break-glass", req("POST", `/access/patients/${rina.id}/break-glass`, { category: "patient_unconscious", reason: "Brought to ED unconscious, need allergies and medication history" }, orthoDoctor.token));

// Bills: Anita's finalised and paid, Mohan's left as a draft.
const finalBill = await must("bill (Anita)", req("POST", "/billing/bills", { patientId: anita.id, billType: "opd" }, billing.token));
let paymentId = null;
if (finalBill) {
  const finalised = await must("finalise", req("POST", `/billing/bills/${finalBill.id}/finalise`, null, billing.token));
  const due = finalised?.balanceDue ?? finalised?.total ?? finalBill.total;
  const paid = await must("payment", req("POST", `/billing/bills/${finalBill.id}/payments`, { amount: Number(due), method: "cash" }, billing.token));
  paymentId = paid?.payment?.id ?? paid?.payments?.at?.(-1)?.id ?? paid?.id ?? null;
  if (!paymentId) {
    const bill = await must("bill after payment", req("GET", `/billing/bills/${finalBill.id}`, null, billing.token));
    paymentId = bill?.payments?.at?.(-1)?.id ?? null;
  }
}
const draftBill = await must("bill (Mohan, draft)", req("POST", "/billing/bills", { patientId: mohan.id, billType: "opd" }, billing.token));
...
async function newPage(options = {}) {
  const ctx = await browser.newContext({ viewport: { width: 1400, height: 1000 }, ...options });
...
async function settle(p, min = 1500) {
  ...
  while (Date.now() < end) {
    const loading = await p.evaluate(() =>
      [...document.querySelectorAll('[aria-label="Loading"], [role="progressbar"]')].some((e) => e.checkVisibility?.() ?? true),
    ).catch(() => false);
    if (!loading) break;
...
const signIn = async (p, email, password) => {
  const statuses = [];
  const onResponse = (r) => {
    if (/\/api\/v1\/auth\//.test(r.url())) statuses.push(`${r.status()} ${new URL(r.url()).pathname.replace("/api/v1", "")}`);
  };
  p.on("response", onResponse);
  try {
    for (let attempt = 1; attempt <= 2; attempt++) {
      await p.goto(WEB, { waitUntil: "networkidle" });
      await p.getByTestId("login-email").waitFor({ state: "visible", timeout: 20_000 });
      ...
      if (left) break;
      if (attempt === 2) {
        const said = await p.locator('[role="alert"]').allInnerTexts().catch(() => []);
        await shot(p, `signin-${email}`);
        throw new Error(`could not sign in as ${email} — page said: ${said.join(" | ") || "nothing"}; auth responses: ${statuses.join(", ") || "none"}`);
      }
    }
  } finally {
...
async function structure(p, { signedIn }) {
  return p.evaluate(({ signedIn }) => {
    const visible = (el) => {
      if (!(el.checkVisibility?.({ checkOpacity: false, checkVisibilityCSS: true }) ?? true)) return false;
    ...
    const nameOf = (el) => {
      ...
      if (el.tagName === "INPUT" || el.tagName === "TEXTAREA") {
        return (el.labels?.[0]?.textContent || el.getAttribute("title") || "").trim();
      }
    ...
    const levels = headings.map((h) => (/^H\d$/.test(h.tagName) ? Number(h.tagName[1]) : Number(h.getAttribute("aria-level") || 2)));
    // A dialog is its own document for this purpose.
    const inDialog = headings.map((h) => Boolean(h.closest('[role="dialog"]')));
    ...
    for (let i = 1; i < pageLevels.length; i++) {
      if (pageLevels[i] > pageLevels[i - 1] + 1) {
        problems.push(`heading skips from h${pageLevels[i - 1]} to h${pageLevels[i]}`);
        break;
      }
    }

    const mains = [...document.querySelectorAll('main, [role="main"]')].filter(visible);
    if (mains.length !== 1) problems.push(`${mains.length} visible main landmarks`);
    if (signedIn && window.innerWidth >= 900) {
      const navs = [...document.querySelectorAll('nav, [role="navigation"]')].filter(visible);
      if (navs.length === 0) problems.push("no navigation landmark around the sidebar");
    }
    ...
    for (const el of document.querySelectorAll("[aria-label]")) {
      ...
      if (/^(Critical|Urgent|Caution|Normal): /.test(label)) {
        ...
        if (!text) problems.push(`signal badge without visible words: ${label}`);
        if (!el.querySelector("svg")) problems.push(`signal badge without its shape: ${label}`);
        ...
        badges.news2++;
        if (!/NEWS2/.test(text) || !/\d/.test(text)) problems.push(`NEWS2 score without its number and name: ${label}`);
      } else if (/^(Critically low|Critically high|Critical|Low|High|Abnormal|Cannot be flagged|No range for this patient)/.test(label) && el.getBoundingClientRect().width < 60) {
        badges.labFlag++;
...
async function audit(p, { screen, role, viewport, signedIn = true }) {
  ...
  for (const v of results.violations) {
    ...
    const entry = byRule.get(v.id) ?? { impact: v.impact, help: v.help, nodes: 0, screens: new Set(), samples: new Set() };
    entry.nodes += v.nodes.length;
    entry.screens.add(`${screen} (${role}, ${viewport})`);
    for (const n of v.nodes.slice(0, 2)) entry.samples.add(`${n.target.join(" ")}  ${(n.failureSummary || "").split("\n").slice(1, 2).join("").trim()}`.slice(0, 220));
    byRule.set(v.id, entry);
    if (v.impact === "serious" || v.impact === "critical") serious += v.nodes.length;
  ...
  cell[viewport] = serious ? `${serious}!` : problems.length ? `s${problems.length}` : minor ? `~${minor}` : "ok";
  row.set(role, cell);
  matrix.set(screen, row);

  if (serious || problems.length) await shot(p, `${role}-${viewport}-${screen}`);
...
async function sidebarRoutes(p) {
  const scope = (await p.locator('nav[aria-label], [role="navigation"]').count()) ? p.locator('nav[aria-label], [role="navigation"]').first() : p.locator("body");
  const names = await scope.getByRole("link").evaluateAll((els) =>
...
const DETAIL = {
  receptionist: [
    { screen: "Patient detail", open: async (p) => { await go(p, "/patients"); await p.getByText("Anita Case", { exact: true }).first().click(); await settle(p); } },
...
try {
  ...
  if (runs("axe")) {
    ...
    for (const vp of VIEWPORTS) {
      const p = await newPage({ viewport: { width: vp.width, height: vp.height } });
      current = p;
      await p.goto(WEB, { waitUntil: "networkidle" });
      await settle(p, 1300);
      let r = await audit(p, { screen: "Login", role: "signed out", viewport: vp.name, signedIn: false });
      check(r.serious === 0, `Login (${vp.name}): no serious or critical axe violations`, `${r.serious} nodes`);

      await p.getByText(/forgot/i).first().click();
      await settle(p, 1200);
      r = await audit(p, { screen: "Forgot password", role: "signed out", viewport: vp.name, signedIn: false });
      check(r.serious === 0, `Forgot password (${vp.name}): no serious or critical axe violations`, `${r.serious} nodes`);

      await signIn(p, newStarter.email, newStarter.password);
      await settle(p);
      r = await audit(p, { screen: "Change password", role: "signed out", viewport: vp.name, signedIn: false });
      check(r.serious === 0, `Change password (${vp.name}): no serious or critical axe violations`, `${r.serious} nodes`);
    ...
    for (const [role, user] of Object.entries(users)) {
      ...
      const targets = [...routes.map((r) => ({ screen: r.name, path: r.path })), ...DETAIL[role]];
      ...
      for (const vp of VIEWPORTS) {
        ...
        for (const t of targets) {
          try {
            ...
            const r = await audit(p, { screen: t.screen, role, viewport: vp.name });
            serious += r.serious;
            problems += r.problems.length;
            if (r.serious || r.problems.length) {
              console.log(`  …     ${t.screen}: ${r.serious} serious/critical node(s) [${r.rules.join(", ")}]${r.problems.length ? ` · ${r.problems.slice(0, 3).join("; ")}` : ""}`);
            }
          } catch (err) {
            warn(`${role} ${vp.name}: could not open ${t.screen} — ${err.message.split("\n")[0]}`);
          }
        }
        check(serious === 0, `${role} (${vp.name}): ${targets.length} screens, no serious or critical axe violations`, `${serious} nodes`);
        check(problems === 0, `${role} (${vp.name}): names, headings, landmarks and signal words hold on every screen`, `${problems} problem(s)`);
      }
      await p.context().close();
    }

    console.log("\naxe — clinical signal badges seen\n");
    check(badgeTotals.signal > 0 && badgeTotals.esi > 0 && badgeTotals.news2 > 0 && badgeTotals.labFlag > 0,
      "SignalBadge, EsiBadge, News2Score and LabFlag were all rendered and checked for words", JSON.stringify(badgeTotals));
  }
  ...
  if (runs("keyboard")) {
    ...
    const tabTo = async (p, testId, { max = 160, onStop } = {}) => {
      ...
      for (let i = 0; i < max; i++) {
        ...
        const stop = await p.evaluate(() => {
          ...
          if (!outline && !ring && (el.tagName === "INPUT" || el.tagName === "TEXTAREA")) {
            for (let a = el.parentElement, depth = 0; a && depth < 3; a = a.parentElement, depth++) {
              const s = getComputedStyle(a);
              if (parseFloat(s.borderTopWidth) >= 2 && s.borderTopColor === "rgb(20, 99, 166)") ring = true;
            }
          }
          return {
            ...
            bracket: el.getAttribute("role") === "none" && el.tabIndex === 0 && !el.childElementCount,
          };
        });
        if (!stop) continue;
        if (!stop.visibleFocus && !stop.bracket) invisible.add(stop.what);
        if (onStop) await onStop(stop);
        if (stop.testId === testId) return { reached: true, tabs: i + 1, invisible: [...invisible] };
      }
      return { reached: false, tabs: max, invisible: [...invisible] };
    };
    const report = async (p, label, result) => {
      check(result.reached, `${label}: Tab reaches it`, `not within ${result.tabs} presses`);
      check(result.invisible.length === 0, `${label}: every Tab stop shows focus`, result.invisible.slice(0, 4).join(" | "));
      if (!result.reached || result.invisible.length) await shot(p, `keyboard-${label}`);
    };
    ...
    await p.goto(WEB, { waitUntil: "networkidle" });
    await settle(p, 1300);
    await report(p, "login sign in", await tabTo(p, "login-submit", { max: 30 }));
    ...
    const announced = await p.evaluate(() =>
      [...document.querySelectorAll('[role="alert"], [aria-live="polite"], [aria-live="assertive"]')]
        .filter((e) => e.checkVisibility?.() ?? true)
        .map((e) => e.innerText.trim())
        .filter(Boolean),
    );
    check(announced.length > 0, "login: a failed sign-in is announced (role=alert or aria-live)", "no live region with text");
    await p.context().close();
    ...
    const fieldErrors = await p.evaluate(() => {
      ...
      const live = [...document.querySelectorAll('[role="alert"], [aria-live]')].filter((e) => e.innerText.trim());
      return { invalid: inputs.length, described: described.length, live: live.length };
    });
    check(fieldErrors.invalid > 0 && fieldErrors.described === fieldErrors.invalid, "register patient: invalid fields are marked aria-invalid and described by their error", JSON.stringify(fieldErrors));
    check(fieldErrors.live > 0, "register patient: field errors sit in a live region", JSON.stringify(fieldErrors));
    await p.context().close();
    ...
    let acked = null;
    await report(p, "dispense", await tabTo(p, "dispense-confirm", {
      onStop: async (stop) => {
        if (stop.testId === "dispense-ack" && acked === null) {
          const before = await p.getByTestId("dispense-ack").getAttribute("aria-checked");
          await p.keyboard.press("Space");
          await p.waitForTimeout(400);
          acked = { before, after: await p.getByTestId("dispense-ack").getAttribute("aria-checked") };
        }
      },
    }));
    check(acked?.before === "false" && acked?.after === "true", "dispense: the allergy acknowledgement is reachable, ticked with Space, and says so (aria-checked)", JSON.stringify(acked));
    await p.context().close();
    ...
    await p.waitForTimeout(1800);
    const picked = await tabTo(p, `generate-pick-${kiran.patientId}`, { max: 20 });
    if (picked.reached) {
      await p.keyboard.press("Enter");
      await settle(p, 2200);
    }
    const gen = await tabTo(p, "generate-opd", { max: 80 });
    check(picked.reached, "generate bill: the patient is chosen from the keyboard");
    ...
    const inDialog = (p) => p.evaluate(() => {
      const dialogs = [...document.querySelectorAll('[role="dialog"], [aria-modal="true"]')];
      const el = document.activeElement;
      return dialogs.length > 0 && dialogs.some((d) => d.contains(el));
    });
    const dialogOpen = (p) => p.evaluate(() => [...document.querySelectorAll('[role="dialog"]')].some((d) => d.checkVisibility?.() ?? true));
    ...
    const trap = async (p, label, trigger) => {
      ...
      check(inside === presses, `${label}: Tab and Shift+Tab stay inside while open`, `${inside}/${presses} inside`);
      await p.keyboard.press("Escape");
      await p.waitForTimeout(900);
      check(!(await dialogOpen(p)), `${label}: Escape closes it`);
      const returned = await p.evaluate((el) => document.activeElement === el || el.contains(document.activeElement), handle);
    ...
    await settle(p);
    await trap(p, "Select sheet", p.getByTestId("user-role").getByRole("button"));
    await go(p, "/admin/users");
    await p.getByTestId("user-row-DOC001").click();
    await settle(p);
    await trap(p, "ConfirmDialog", p.getByTestId("user-deactivate"));
    check(await p.getByTestId("user-deactivate").isVisible(), "ConfirmDialog: Escape cancelled — the account is still active");
    await p.context().close();
    ...
    await p.route("**/api/v1/nursing/observations", async (route) => {
      ...
      return route.fulfill({ status: 422, contentType: "application/json", body: JSON.stringify({ success: false, message: "This admission has been discharged" }) });
    });
    await p.context().setOffline(true);
    await p.getByText("Record obs", { exact: true }).first().click();
    await p.waitForTimeout(600);
    for (const [id, v] of Object.entries({ respiratoryRate: 18, spo2: 96, systolic: 122, pulse: 84, temperatureC: 37.2 })) await p.getByTestId(`obs-${id}`).fill(String(v));
    ...
    const reviewVisible = await p.getByTestId("outbox-review").waitFor({ state: "visible", timeout: 40_000 }).then(() => true).catch(() => false);
    if (check(reviewVisible, "outbox review: a refused offline entry offers Review")) {
  ...
  if (runs("reflow")) {
    ...
    const REFLOW = [
      ...
      { role: "receptionist", screen: "Register patient", path: "/patients/new" },
      { role: "receptionist", screen: "Patients", path: "/patients" },
      { role: "doctor", screen: "Medical record", path: `/patients/${anita.id}/record` },
      { role: "nurse", screen: "Bedside", path: `/nursing/patients/${admission.id}` },
      { role: "nurse", screen: "Emergency board", path: "/emergency" },
      { role: "lab", screen: "Lab order", path: `/lab/requests/${rft?.id}` },
      { role: "pharmacy", screen: "Dispense", path: `/pharmacy/dispense/${rxPending?.id}` },
      { role: "billing", screen: "Bill detail", path: `/billing/bills/${finalBill?.id}` },
    ...
    for (const t of REFLOW) {
      ...
      if (!p) {
        p = await newPage();
        if (t.role) await signIn(p, users[t.role].email, users[t.role].password);
        pages.set(t.role, p);
      }
      current = p;
      for (const width of [320, 640]) {
        ...
        const r = await p.evaluate(() => {
          ...
          for (let n = walker.nextNode(); n; n = walker.nextNode()) {
            ...
            if (rect.right > window.innerWidth + 1 && !scrollsX(el)) lost.push(n.textContent.trim().slice(0, 30));
          }
          return { pageScroll, lost: lost.slice(0, 5), lostCount: lost.length };
        });
        const ok = check(r.pageScroll <= 1 && r.lostCount === 0, `${t.screen} at ${width}px: no page scroll sideways, nothing off-screen`,
  ...
  if (runs("motion")) {
    ...
    const sample = async (reducedMotion) => {
      ...
      await p.keyboard.press("Enter");
      const cssAnimations = await p.evaluate(() => document.getAnimations().length);
      await p.keyboard.press("Escape");
      await p.waitForTimeout(600);
      await p.context().close();
      ...
      await q.route("**/api/v1/pharmacy/prescriptions/*/dispense-context", async (route) => {
        await new Promise((r) => setTimeout(r, 6000));
        await route.fallback().catch(() => {});
      });
      await q.goto(`${WEB}/pharmacy/dispense/${rxPending.id}`, { waitUntil: "domcontentloaded" });
      await q.waitForTimeout(2000);
      const opacities = [];
      for (let i = 0; i < 6; i++) {
        opacities.push(await q.evaluate(() => {
          // Skeleton: ink-200 fill, the 100 px block at the top of the screen.
          const s = [...document.querySelectorAll("div")].find(
            (d) => getComputedStyle(d).backgroundColor === "rgb(220, 225, 231)" && d.getBoundingClientRect().height >= 90,
          );
    ...
    console.log(`  (no-preference: ${normal.cssAnimations} running CSS animation(s) opening a sheet, skeleton opacity ${normal.opacities.join(" → ")})`);
    console.log(`  (reduce:        ${reduced.cssAnimations} running CSS animation(s) opening a sheet, skeleton opacity ${reduced.opacities.join(" → ")})`);
    check(reduced.cssAnimations === 0, "reduced motion: a sheet opens without an animation");
    const seen = reduced.opacities.filter((o) => o !== null);
    if (seen.length >= 2) check(new Set(seen).size === 1, "reduced motion: loading skeletons hold still", seen.join(" → "));
    else warn("reduced motion: no loading skeleton was on screen long enough to sample");
    if (normal.cssAnimations === 0 && new Set(normal.opacities.filter(Boolean)).size <= 1) warn("reduced motion: nothing animated even without the preference — the check proved little");
  }
} catch (err) {
  console.error("\nGate threw:", err.message);
  failures.push(`exception: ${err.message.split("\n")[0]}`);
  if (current) await current.screenshot({ path: path.join(SHOTS, "a11y-FAILURE.png") }).catch(() => {});
} finally {
  await browser.close();
  web.close();
  api.kill("SIGTERM");
...
if (byRule.size) {
  ...
  for (const [rule, e] of rows) {
    const level = e.impact === "serious" || e.impact === "critical" ? "FAIL" : "warn";
    console.log(`  ${level}  ${rule.padEnd(28)} ${String(e.impact).padEnd(9)} ${String(e.nodes).padStart(4)} node(s) on ${e.screens.size} render(s) — ${e.help}`);
    for (const s of [...e.samples].slice(0, 2)) console.log(`          e.g. ${s}`);
...
if (structureProblems.size) {
  console.log("\nStructure problems\n");
  for (const [pr, screens] of structureProblems) console.log(`  FAIL  ${pr} — ${[...screens].slice(0, 4).join("; ")}${screens.size > 4 ? ` (+${screens.size - 4})` : ""}`);
}

if (matrix.size) {
  const roles = ["signed out", ...Object.keys(users)];
  const cols = roles.filter((r) => [...matrix.values()].some((row) => row.has(r)));
  const short = { "signed out": "out", receptionist: "rec", doctor: "doc", nurse: "nur", lab: "lab", pharmacy: "pha", billing: "bil", inventory: "inv", admin: "adm" };
  ...
  console.log(`  ${"Screen".padEnd(w)}${cols.map((c) => short[c].padEnd(12)).join("")}`);
  for (const [screen, row] of [...matrix.entries()].sort()) {
    const cells = cols.map((c) => {
      const cell = row.get(c);
      return (cell ? `${cell.desktop ?? "-"}/${cell.phone ?? "-"}` : "").padEnd(12);
    });
    console.log(`  ${screen.padEnd(w)}${cells.join("")}`);
  }
  const renders = [...matrix.values()].reduce((n, row) => n + [...row.values()].reduce((m, c) => m + Object.keys(c).length, 0), 0);
...
if (process.env.A11Y_JSON) {
  fs.writeFileSync(process.env.A11Y_JSON, JSON.stringify({
    rules: Object.fromEntries([...byRule].map(([k, v]) => [k, { impact: v.impact, nodes: v.nodes, renders: v.screens.size, samples: [...v.samples] }])),
    structure: Object.fromEntries([...structureProblems].map(([k, v]) => [k, [...v]])),
    badges: badgeTotals, failures, warnings, seedProblems, screensAudited,
  }, null, 2));
}

const jsErrors = consoleErrors.filter((e) => !/Failed to load resource|net::ERR_/i.test(e));
if (jsErrors.length) warn(`JavaScript errors on the page: ${[...new Set(jsErrors)].slice(0, 2).join(" | ")}`);
