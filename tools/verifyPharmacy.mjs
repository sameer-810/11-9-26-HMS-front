...
const { StockBatchModel } = await imp(BACK, "src", "modules", "inventory", "inventory.model.js");

const hospital = await HospitalModel.create({
  name: "City General Hospital", code: "CGH", approvalStatus: "approved", approvedAt: new Date(), isActive: true, timezone: "Asia/Kolkata",
});
...
const adminToken = (
  await req("POST", "/auth/login", { email: "admin@cgh.test", password: "AdminPassword123", deviceId: "verify-admin-device", deviceName: "Verifier" })
).data.accessToken;
...
const amoxil = await req("POST", "/prescriptions/medicines", { name: "Amoxil", genericName: "Amoxicillin", ingredients: ["amoxicillin"], form: "capsule", strength: "500mg" }, adminToken);
const amoxItem = await req("POST", "/inventory/items", { code: "AMOX500", name: "Amoxil 500mg capsule", medicineId: amoxil.data.id, unit: "capsule", reorderLevel: 20 }, store.token);
...
const mkPatient = async (firstName, mobile) => {
  const p = await req("POST", "/patients", { firstName, lastName: "Case", gender: "female", dateOfBirth: "1975-05-05", mobile }, reception.token);
  await req("PUT", `/patients/${p.data.id}/allergies`, { allergies: [] }, doctor.token);
  return p.data;
};
const anita = await mkPatient("Anita", "9876500001");
const bhavna = await mkPatient("Bhavna", "9876500002");

console.log("Seeded: Amoxil in the formulary, a stock item, a supplier and expired stock on the pharmacy shelf\n");
...
try {
  ...
  await storePage.goto(`${WEB}/inventory/receive`, { waitUntil: "networkidle" });
  ...
  check((await body(storePage)).includes("a two-digit year is not accepted"), "IN-01: a two-digit expiry year is refused as it is typed");
  check(await storePage.getByTestId("receive-submit").isDisabled(), "and the delivery cannot be submitted with it");
  await storePage.screenshot({ path: path.join(SHOTS, "supply-1-receive-bad-expiry.png") });

  await storePage.getByTestId("receive-expiry-0").fill("03/2030");
  await storePage.waitForTimeout(400);
  check((await body(storePage)).includes("Usable until 31 Mar 2030"), "a month-and-year expiry is read back as the last day of the month");

  await storePage.getByTestId("receive-submit").click();
  await storePage.waitForTimeout(2400);
  check(/GRN-\d{6} recorded/.test(await storePage.getByTestId("receive-success").innerText()), "the receipt is recorded with its goods-received number");
  ...
  await storePage.waitForTimeout(400);
  check((await body(storePage)).includes("Only 200 in this batch"), "IN-02: issuing more than the batch holds is refused on screen");
  check(await storePage.getByTestId("issue-submit").isDisabled(), "and cannot be submitted");
  ...
  await storePage.waitForTimeout(2400);
  check(/ISS-\d{6} recorded/.test(await storePage.getByTestId("issue-success").innerText()), "the transfer is recorded");
  await storePage.screenshot({ path: path.join(SHOTS, "supply-2-transfer.png") });

  await storePage.goto(`${WEB}/inventory`, { waitUntil: "networkidle" });
  await storePage.waitForTimeout(2400);
  check((await storePage.getByTestId("item-AMOX500-pharmacy").innerText()).startsWith("150"), "the pharmacy now holds 150 usable");
  check((await storePage.getByTestId("item-AMOX500-expired").innerText()) === "50", "and the 50 expired are counted separately, never added in");
  await storePage.screenshot({ path: path.join(SHOTS, "supply-3-inventory.png") });

  // =========================================================================
  console.log("\nPH-01..05 — the pharmacist dispenses\n");

  const rxA = (await req("POST", "/prescriptions", {
    patientId: anita.id, urgency: "urgent",
    lines: [{ medicineId: amoxil.data.id, dose: "1 cap", frequency: "tds", durationDays: 5 }],
  }, doctor.token)).data.prescription;

  const counter = await newPage();
  await signIn(counter, pharmacist.email, pharmacist.password);
  await counter.goto(`${WEB}/pharmacy/prescriptions`, { waitUntil: "networkidle" });
  await counter.waitForTimeout(2400);
  check(await counter.getByTestId(`rx-row-${rxA.prescriptionNumber}`).isVisible(), "PH-01: the prescription is in the pharmacy queue");
  ...
  check(text.includes("No known allergies"), "PH-03: the allergy status is on the dispensing screen");
  check(text.includes("In stock"), "PH-02: stock is shown against live pharmacy inventory");
  check(
    (await counter.getByTestId("locked-P-OLD").innerText()).includes("Expired — cannot be selected"),
    "PH-04: the expired batch is on screen, and locked",
  );
  check((await counter.getByTestId("qty-A1").inputValue()) === "15", "the usable batch is suggested for the whole course");
  check(await counter.getByTestId("dispense-confirm").isDisabled(), "PH-03: confirm is disabled until the allergy check is ticked");
  ...
  check(!(await counter.getByTestId("dispense-confirm").isDisabled()), "and enabled once it is");
  ...
  check(text.includes(rxA.prescriptionNumber), "IN-04: the store's ledger shows the dispense against its prescription number");
  check(!text.includes("Anita"), "and never the patient's name");

  // =========================================================================
  console.log("\nPH-03 — an allergy recorded while the prescription is open\n");

  const rxB = (await req("POST", "/prescriptions", {
    patientId: bhavna.id,
    lines: [{ medicineId: amoxil.data.id, dose: "1 cap", frequency: "tds", durationDays: 5 }],
  }, doctor.token)).data.prescription;

  await counter.goto(`${WEB}/pharmacy/dispense/${rxB.id}`, { waitUntil: "networkidle" });
  await counter.waitForTimeout(2600);
  check((await body(counter)).includes("No known allergies"), "the pharmacist opens it: no known allergies");
  await counter.getByTestId("dispense-ack").click();
  await counter.waitForTimeout(300);
  ...
  text = await body(counter);
  check((await counter.getByTestId("dispense-error").innerText()).includes("allergy list changed"), "THE CASE: the dispense is refused — the list she ticked is no longer true");
  check(text.includes("Penicillin · anaphylaxis"), "the screen reloads with the new allergy");
  check(await counter.getByTestId("allergies-changed").isVisible(), "and says it changed since the prescription was written");
  check(await counter.getByTestId("dispense-blocked-Amoxil").isVisible(), "the amoxicillin is locked: the prescriber never considered this allergy");
  check((await counter.getByTestId("dispense-ack").getAttribute("aria-checked")) !== "true", "and the old tick is gone");
  check(await counter.getByTestId("dispense-confirm").isDisabled(), "so nothing can be confirmed");
  await counter.screenshot({ path: path.join(SHOTS, "supply-6-allergy-changed.png") });

  await counter.getByTestId("dispense-ack").click();
  await counter.waitForTimeout(300);
  check(await counter.getByTestId("dispense-confirm").isDisabled(), "even ticking it again cannot release a blocked medicine");

  await counter.goto(`${WEB}/pharmacy/prescriptions`, { waitUntil: "networkidle" });
  await counter.waitForTimeout(2400);
  check(await counter.getByTestId(`rx-allergy-changed-${rxB.prescriptionNumber}`).isVisible(), "the queue flags it before anyone opens it again");

  // =========================================================================
  await storePage.goto(`${WEB}/dashboard`, { waitUntil: "networkidle" });
  await storePage.waitForTimeout(2200);
  check((await body(storePage)).includes("Expired on the shelf"), "IN-03: the store's dashboard shows expired stock on the shelf");
  ...
  await storePage.screenshot({ path: path.join(SHOTS, "supply-FAILURE.png") }).catch(() => {});
} finally {
  await browser.close();
  web.close();
  api.kill("SIGTERM");
