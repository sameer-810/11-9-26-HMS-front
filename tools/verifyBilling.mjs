...
for (const to of ["sample_collected", "in_progress"])
  await req("POST", `/laboratory/orders/${cbc.id}/stage`, { to }, lab.token);
await req(
  "PUT",
  `/laboratory/orders/${cbc.id}/results`,
...
const med = async (name, ingredient) =>
  (
    await req(
      "POST",
...
const item = async (code, medicineId) =>
  (
    await req(
      "POST",
      "/inventory/items",
      { code, name: code, medicineId, unit: "tablet" },
      store.token,
    )
  ).data.id;
...
const ctx = (
  await req(
    "GET",
    `/pharmacy/prescriptions/${rx.id}/dispense-context`,
    null,
    pharmacist.token,
  )
).data;
await req(
  "POST",
  `/pharmacy/prescriptions/${rx.id}/dispense`,
  {
...
console.log(
  "Seeded: a signed consultation, a reported blood test, two dispensed medicines (one unpriced) and a tariff\n",
);
...
try {
  ...
  check(
    text.includes("Consultation — General Medicine"),
    "BL-01: the signed consultation is a charge",
  );
  check(
    text.includes("Complete blood count"),
    "the reported blood test is a charge",
  );
  check(
    text.includes("Glycomet 500mg") && text.includes("Calcirol 500mg"),
    "the dispensed medicines are charges",
  );
  check(
    text.includes("Not priced"),
    "an unpriced medicine is marked, not charged as free",
  );
  text = await body(counter);
  check(
    !text.includes("diabetes") && !text.includes("anaemia"),
    "section 6.6: no diagnosis or indication on the billing screen",
  );
  check(!text.includes("Sulfa"), "and no allergies");
  await counter.screenshot({
    path: path.join(SHOTS, "billing-1-compiled.png"),
  });

  await counter.getByTestId("generate-opd").click();
  await counter.waitForTimeout(2800);
  check(
    (await counter.getByTestId("bill-status").innerText()) === "Draft",
    "BL-02: the bill is generated as a draft",
  );
  const billId = counter.url().split("/billing/bills/")[1]?.split("?")[0];
  check(Boolean(billId), "the bill has its own address");
  check(
    (await body(counter)).includes("A charge has no price"),
    "the unpriced charge is called out on the draft",
  );
  ...
  check(
    text.includes("Calcirol") && text.includes("never charged as free"),
    "finalising with an unpriced charge is refused, and says why",
  );
  ...
  check(
    !(await body(counter)).includes("A charge has no price"),
    "once priced by administration, a refresh prices the line",
  );
  check(
    (await counter.getByTestId("bill-total").innerText()).includes("900"),
    "the total is 500 + 350 + 30 + 20",
  );
  ...
  await counter.waitForTimeout(2400);
  check(
    (await counter.getByTestId("bill-total").innerText()).includes("1,200"),
    "a procedure from the tariff adds its tariff price",
  );

  await counter.getByTestId("remove-Glycomet 500mg").click();
  await counter.waitForTimeout(500);
  await counter
    .getByTestId("remove-reason")
    .fill("Patient paying for medicines separately");
  await counter.getByTestId("remove-submit").click();
  await counter.waitForTimeout(2400);
  check(
    (await counter.getByTestId("bill-total").innerText()).includes("1,170"),
    "a charge is removed from the draft with a reason",
  );
  check(
    (await body(counter)).includes("Removed Glycomet 500mg"),
    "and the removal stays on the bill",
  );
  ...
  await counter
    .getByTestId("discount-reason")
    .fill("Senior citizen concession, policy FIN-4");
  await counter.getByTestId("discount-submit").click();
  await counter.waitForTimeout(2400);
  check(
    await counter.getByTestId("discount-pending").isVisible(),
    "the discount is requested, not applied",
  );
  check(
    (await counter.getByTestId("bill-total").innerText()).includes("1,170"),
    "the total does not change until it is approved",
  );
  check(
    (await counter.getByTestId("discount-approve").count()) === 0,
    "billing is not offered the approval",
  );
  await counter.screenshot({
    path: path.join(SHOTS, "billing-2-discount-pending.png"),
  });

  const office = await newPage();
  await signIn(office, "admin@cgh.test", "AdminPassword123");
  await office.goto(`${WEB}/billing/bills/${billId}`, {
    waitUntil: "networkidle",
  });
  await office.waitForTimeout(2600);
  check(
    await office.getByTestId("discount-decision").isVisible(),
    "administration sees the request with its reason",
  );
  await office.getByTestId("discount-approve").click();
  await office.waitForTimeout(2400);
  check(
    (await office.getByTestId("bill-discount").innerText()).includes("−"),
    "and approves it",
  );
  ...
  await counter.waitForTimeout(2600);
  check(
    (await counter.getByTestId("bill-total").innerText()).includes("970"),
    "the approved discount comes off the total",
  );
  await counter.getByTestId("finalise-open").click();
  await counter.waitForTimeout(600);
  await counter.getByText("Finalise", { exact: true }).last().click();
  await counter.waitForTimeout(2800);
  check(
    (await counter.getByTestId("bill-status").innerText()) === "Finalised",
    "BL-02: the bill is finalised",
  );
  check(
    (await counter.locator('[data-testid^="remove-"]').count()) === 0,
    "section 7: every charge control is gone",
  );
  check(
    (await counter.getByTestId("draft-actions").count()) === 0,
    "no service, discount or refresh is offered",
  );
  await counter.screenshot({
    path: path.join(SHOTS, "billing-3-finalised.png"),
  });
  ...
  check(
    (await counter.getByTestId("pay-amount").inputValue()) === "970",
    "the payment form starts at the balance",
  );
  await counter.getByTestId("pay-amount").fill("5000");
  await counter.waitForTimeout(400);
  check(
    (await body(counter)).includes("More than the balance"),
    "BL-03: more than the balance is refused on screen",
  );
  check(
    await counter.getByTestId("pay-submit").isDisabled(),
  ...
  check(
    await counter.getByTestId("pay-submit").isDisabled(),
    "a card payment needs its reference",
  );
  ...
  await counter.waitForTimeout(2800);
  check(
    (await counter.getByTestId("bill-status").innerText()) === "Partially paid",
    "BL-03: a partial payment makes the bill partially paid",
  );
  check(
    (await counter.getByTestId("bill-balance").innerText()).includes("770"),
    "and the balance falls to 770",
  );

  // BL-05, while it is still owed.
  await counter.goto(`${WEB}/billing/outstanding`, {
    waitUntil: "networkidle",
  });
  await counter.waitForTimeout(2400);
  ...
  check(
    text.includes("Anita Case") && text.includes("770"),
    "BL-05: the unpaid bill is outstanding with patient and amount",
  );
  check(text.includes("0 days"), "and its age");
  await counter.screenshot({
    path: path.join(SHOTS, "billing-4-outstanding.png"),
  });
  ...
  await counter.waitForTimeout(2800);
  check(
    (await counter.getByTestId("bill-status").innerText()) === "Paid",
    "the rest by UPI marks the bill paid",
  );
  check(
    (await counter.getByTestId("payment-form").count()) === 0,
    "and a paid bill offers no payment form",
  );
  ...
  await counter.waitForTimeout(2400);
  check(
    (await counter.getByTestId("receipt-words").innerText()) ===
      "Rupees Two Hundred Only",
    "BL-04: the receipt gives the amount in words",
  );
  text = await counter.getByTestId("receipt").innerText();
  check(
    text.includes("GSTIN 29ABCDE1234F1Z5") &&
      text.includes("City General Hospital"),
    "with the hospital's name and GSTIN",
  );
  check(
    !text.includes("Sulfa") && !text.includes("diabetes"),
  ...
  const unexpected = httpFailures.filter(
    (f) => !/^409 POST \/api\/v1\/billing\/bills\/[0-9a-f]+\/finalise$/.test(f),
  );
  check(
    unexpected.length === 0,
  ...
  await counter
    .screenshot({ path: path.join(SHOTS, "billing-FAILURE.png") })
    .catch(() => {});
} finally {
  await browser.close();
  web.close();
