...
await req(
  "POST",
  `/emergency/visits/${anitaVisit.id}/triage`,
...
const rowOrder = async (p) =>
  p
    .locator('[data-testid^="ed-row-"]')
    .evaluateAll((els) =>
      els.map((e) => e.getAttribute("data-testid").replace("ed-row-", "")),
    );
...
try {
  ...
  await desk
    .getByTestId("register-chief-complaint")
    .fill("Chest pain radiating to the left arm");
  ...
  check(
    Boolean(bhavnaNumber),
    "a registered patient's arrival gets an ED number",
  );
  check(
    (await desk.getByTestId("triage-form").count()) === 0,
    "reception is not offered the triage form",
  );
  ...
  await desk
    .getByTestId("register-chief-complaint")
    .fill("Road traffic accident, unconscious");
  ...
  check(
    Boolean(unknownNumber) && (await body(desk)).includes("Unidentified"),
    "an unidentified ambulance arrival is registered without a name",
  );
  ...
  check(
    order[order.length - 1] === anitaVisit.visitNumber,
    "the first arrival, ESI 5, is at the bottom",
    order.join(", "),
  );
  check(
    (await ward.getByTestId(`ed-esi-${bhavnaNumber}`).innerText()).includes(
      "Triage now",
    ),
    "an untriaged patient is marked for triage",
  );
  ...
  check(
    (await ward.getByTestId("triage-preview-level").innerText()).includes(
      "ESI 2",
    ),
    "a high-risk presentation suggests ESI 2",
  );
  ...
  check(
    (await ward.getByTestId("ed-visit-esi").innerText()).includes("ESI 2"),
    "the nurse accepts it and the attendance is ESI 2",
  );
  ...
  check(
    order.join() ===
      [unknownNumber, bhavnaNumber, anitaVisit.visitNumber].join(),
  ...
  check(
    await ward.getByTestId("triage-save").isDisabled(),
    "an override cannot be saved without a reason",
  );
  await ward
    .getByTestId("triage-override-reason")
    .fill("Diabetic, foot is cold and pale below the injury");
  await ward.waitForTimeout(400);
  await ward.getByTestId("triage-save").click();
  await ward.waitForTimeout(2600);
  check(
    (await ward.getByTestId("ed-triage-override").innerText()).includes(
      "suggested",
    ),
    "the override keeps the algorithm's suggestion beside it",
  );

  await go(desk, `/emergency`);
  check(
    (await desk.getByTestId(`ed-esi-${bhavnaNumber}`).innerText()).includes(
      "ESI 2",
    ),
    "reception sees the level on the board",
  );
  ...
  check(
    await clinic.getByTestId("ed-open-consultation").isVisible(),
    "taking the patient opens an emergency consultation",
  );

  await go(clinic, "/dashboard");
  check(
    (await body(clinic)).includes("In emergency"),
    "the department shows on the doctor's dashboard",
  );
  ...
  check(
    await glass.getByTestId("record-restricted").isVisible(),
    "a clinician outside the team meets the restriction, not the record",
  );
  check(
    (await glass.getByTestId("medical-record").count()) === 0,
    "and nothing of the record is shown",
  );
  await glass.getByTestId("breakglass-reason").fill("urgent");
  await glass.waitForTimeout(300);
  check(
    await glass.getByTestId("breakglass-submit").isDisabled(),
    "the glass does not break without a real reason",
  );
  await glass.screenshot({
    path: path.join(SHOTS, "emergency-3-restricted.png"),
  });

  await glass
    .getByTestId("breakglass-reason")
    .fill("Brought to ED unconscious, need allergies and medication history");
  await glass.waitForTimeout(300);
  await glass.getByTestId("breakglass-submit").click();
  await glass.waitForTimeout(3200);
  check(
    await glass.getByTestId("medical-record").isVisible(),
    "with a reason, the record opens",
  );
  text = await glass.getByTestId("breakglass-active").innerText();
  check(
    text.includes("Emergency access") &&
      /Access ends in (59|60) min/.test(text),
    "under a banner counting down its sixty minutes",
    text,
  );
  await glass.screenshot({
    path: path.join(SHOTS, "emergency-4-glass-broken.png"),
  });
  ...
  check(
    text.includes("EMERGENCY ACCESS") && text.includes("unconscious"),
    "the audit trail flags emergency access with its reason",
  );
  ...
  check(
    text.includes("Farah") && text.includes("Awaiting review"),
    "the grant waits for review, naming who used it",
  );
  const grantId = (
    await office
      .locator('[data-testid^="grant-inappropriate-"]')
      .first()
      .getAttribute("data-testid")
  ).replace("grant-inappropriate-", "");
  await office.getByTestId(`grant-inappropriate-${grantId}`).click();
  await office.waitForTimeout(400);
  check(
    await office.getByTestId(`grant-submit-${grantId}`).isDisabled(),
    "an inappropriate finding needs a note",
  );
  await office.getByTestId(`grant-appropriate-${grantId}`).click();
  await office.waitForTimeout(2400);
  check(
    (await body(office)).includes("Nothing awaiting review"),
  ...
  check(
    /Arrivals\s*3/.test(text),
    "the emergency report counts the three arrivals",
    text.slice(0, 120),
  );
  await office.getByTestId("report-preset-last-7-days").click();
  await office.waitForTimeout(1800);
  check(
    (await body(office)).includes("7 days"),
    "a date preset changes the range",
  );
  ...
  check(
    text.includes("General Medicine") &&
      text.includes("What the department filter counts"),
    "a department filter, with what it means",
  );
  check(
    /Arrivals\s*1/.test(await office.getByTestId("report-summary").innerText()),
    "and it changes the numbers: one arrival seen in Medicine",
  );
  for (const name of ["Anita", "Bhavna", "Rina"])
    check(!text.includes(name), `no patient name in the report (${name})`);
  ...
  check(
    /^emergency_\d{4}-\d{2}-\d{2}_\d{4}-\d{2}-\d{2}\.csv$/.test(csvName),
    "the report exports as CSV",
    csvName || "no download",
  );
  ...
  check(
    (await counter.getByTestId("report-chip-billing").count()) === 1,
    "billing staff run the billing report",
  );
  check(
    (await counter.getByTestId("report-chip-emergency").count()) === 0,
  ...
  const tempPassword = (
    await office.getByTestId("user-temp-password").innerText()
  ).trim();
  check(
    tempPassword.length >= 8,
    "a new user is created with a one-time temporary password",
  );
  check(
    (await body(office)).includes("This password will not be shown again"),
    "and told it will not be shown again",
  );
  const firstLogin = await req("POST", "/auth/login", {
    email: "meera@cgh.test",
    password: tempPassword,
    deviceId: "meera-device",
    deviceName: "Verifier",
  });
  check(firstLogin.success === true, "the temporary password signs in");
  await office.screenshot({
    path: path.join(SHOTS, "admin-1-user-created.png"),
  });
  ...
  await office.waitForTimeout(2400);
  check(
    await office.getByTestId("user-activate").isVisible(),
    "the account is deactivated from its page",
  );
  const refused = await req("POST", "/auth/login", {
    email: "meera@cgh.test",
    password: tempPassword,
    deviceId: "meera-device",
    deviceName: "Verifier",
  ...
  await office.waitForTimeout(1800);
  check(
    (await office.getByTestId("department-row-MED").count()) === 1 &&
      (await office.getByTestId("department-row-ORT").count()) === 1,
    "hospital setup lists the departments",
  );

  await go(office, "/beds");
  check(
    (await office.getByTestId("bed-MWA-A1").count()) === 1,
    "the bed board shows the ward's beds",
  );
  await office.locator('[data-testid^="bed-maintenance-"]').first().click();
  await office.waitForTimeout(600);
  await office
    .getByTestId("bed-maintenance-note")
    .fill("Bed rail broken, awaiting repair");
  await office.getByTestId("bed-maintenance-submit").click();
  await office.waitForTimeout(2400);
  check(
    (await office.getByTestId("beds-notice").innerText()).includes(
      "out of service",
    ),
    "a bed is taken out of service with a note",
  );
  ...
  check(
    (await counter.getByTestId("profile-role").innerText()).length > 0,
  ...
  await current
    .screenshot({ path: path.join(SHOTS, "emergency-FAILURE.png") })
    .catch(() => {});
} finally {
  await browser.close();
  web.close();
