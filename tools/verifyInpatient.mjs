...
const nurse = await provision({
  employeeId: "NUR001", firstName: "Meera", email: "meera@cgh.test", role: "nurse",
});
const nightNurse = await provision({
  employeeId: "NUR002", firstName: "Latha", email: "latha@cgh.test", role: "nurse",
});
...
const ward = await req(
  "POST", "/beds/wards",
  { name: "Medical Ward A", code: "MWA", type: "general", departmentId: dept.data.id },
  adminToken,
);
...
await req("POST", "/beds/bulk", { roomId: room.data.id, prefix: "A", from: 1, to: 4 }, adminToken);
...
const patient = await req(
  "POST", "/patients",
  {
    firstName: "Sunita", lastName: "Sharma", gender: "female",
    dateOfBirth: "1968-02-11", mobile: "9876543210",
  },
  reception.token,
);
await req(
...
const enalapril = await req("POST", "/prescriptions/medicines", {
  ...
  defaultDose: "1 tab", defaultFrequency: "bd", defaultDurationDays: 5,
}, adminToken);
...
try {
  ...
  await page.getByTestId("admit-reason").fill(
    "Community acquired pneumonia. Needs IV antibiotics and oxygen.",
  );
  ...
  check(bedside.includes("Sunita Sharma"), "the bedside chart opens on the admitted patient");
  ...
  const nursePage = await (await browser.newContext({ viewport: { width: 1280, height: 1000 } })).newPage();
  ...
  await nursePage.goto(`${WEB}/nursing/patients/${admissionId}`, { waitUntil: "networkidle" });
  ...
  check(
    stillMissing.includes("air or oxygen") && stillMissing.includes("consciousness"),
    "naming the parameters, not just a count",
  );
  await nursePage.screenshot({ path: path.join(SHOTS, "ipd-3-obs-incomplete.png") });
  ...
  check(
    !(await nursePage.innerText("body")).includes("Still needed for a NEWS2 score"),
  ...
  check(scored.includes("NEWS2"), "the score is shown to the nurse who took the readings");
  ...
  await nursePage.screenshot({ path: path.join(SHOTS, "ipd-4-news2-high.png") });
  ...
  check(board.includes("need review"), "the escalation strip is above everything");
  ...
  await page.getByTestId("acknowledge-note").fill(
  ...
  await req(
    "POST", "/prescriptions",
    {
      ...
      lines: [{ medicineId: enalapril.data.id, dose: "1 tab", frequency: "bd", durationDays: 5 }],
    },
    doctor.token,
  );
  ...
  await nursePage.screenshot({ path: path.join(SHOTS, "ipd-7-drug-round.png") });
  ...
  await nightPage.goto(`${WEB}/nursing/patients/${admissionId}`, { waitUntil: "networkidle" });
  ...
  await nightPage.screenshot({ path: path.join(SHOTS, "ipd-8-shift-change.png") });
  ...
  check(
    (await nightPage.getByTestId("omit-reason").getAttribute("placeholder"))?.includes(
      "next shift has to decide",
    ) ?? false,
    "and the form says why it is asking",
  );

  await nightPage.getByTestId("omit-reason").fill(
  ...
  check(
    partial.includes("recommendation"),
    "and the missing part is named",
  );
  check(
    partial.includes("it is the part the next shift acts on"),
    "with the reason it matters, rather than a red asterisk",
  );
  await nursePage.screenshot({ path: path.join(SHOTS, "ipd-9-sbar-incomplete.png") });

  await nursePage.getByTestId("sbar-recommendation").fill(
    "Hourly observations overnight. Chase the blood cultures at 22:00. Call the doctor if NEWS rises above 5.",
  );
  await nursePage.getByTestId("sbar-tasks").fill("Chase blood cultures\nRepeat CRP in the morning");
  await nursePage.getByTestId("sbar-alerts").fill("Poor IV access — single cannula in left ACF");
  await nursePage.waitForTimeout(400);
  ...
  await nursePage.screenshot({ path: path.join(SHOTS, "ipd-10-sbar-given.png") });

  // =========================================================================
  console.log("\nIP-05 — discharge\n");

  await page.goto(`${WEB}/ipd/patients/${admissionId}/discharge`, { waitUntil: "networkidle" });
  ...
  await page.getByTestId("discharge-summary").fill(
  ...
  await page.waitForTimeout(400);
  check(
    (await page.innerText("body")).includes("Still needed: discharge medication"),
    "and the form keeps saying what is left",
  );
  await page.screenshot({ path: path.join(SHOTS, "ipd-11-discharge-incomplete.png") });

  await page.getByTestId("discharge-medication").fill(
    "Amoxicillin 500mg three times a day for 3 more days. Continue enalapril 5mg twice daily.",
  );
  await page.getByTestId("discharge-followup").fill(
    "GP review in one week. Return immediately if breathless, confused, or the fever returns.",
  );
  await page.getByTestId("discharge-diagnosis").fill("Community acquired pneumonia, resolved");
  ...
  check(
    !afterDischarge.includes("Sunita Sharma") || afterDischarge.includes("Nobody is admitted"),
    "she leaves the ward board on discharge",
  );
  await page.screenshot({ path: path.join(SHOTS, "ipd-12-after-discharge.png") });

  // IP-03 again, from the other end: the bed is free without anyone freeing it.
  const bedsAfter = await req(`GET`, `/beds?wardId=${ward.data.id}&limit=20`, null, adminToken);
  ...
  await page.screenshot({ path: path.join(SHOTS, "ipd-FAILURE.png") }).catch(() => {});
} finally {
  await browser.close();
  web.close();
  api.kill("SIGTERM");
