...
import {
  ...
  ptToMm,
  MIN_NARROW_BAR_MM,
} from "../src/shared/print/sizing";
import {
  code128,
  dataMatrix,
  CODE128_QUIET_MODULES,
} from "../src/shared/print/barcode";
import { escapeHtml, html, raw } from "../src/shared/print/escapeHtml";
import {
  patientPayload,
  specimenPayload,
} from "../src/shared/print/scanPayload";
import {
  allergyStatement,
  printedName,
  formatDob,
} from "../src/modules/printing/documents/common";
import {
  buildWristband,
  WRISTBAND_STOCK,
} from "../src/modules/printing/documents/wristband";
...
const serverScanCode = path.resolve(
  here,
  "..",
  "..",
  "11-9-26-HMS-back",
...
test("Code 128 is drawn exactly modules × module width wide", () => {
  ...
  assert.ok(
    Math.abs(sym.quietMm - CODE128_QUIET_MODULES * sym.moduleMm) < 1e-9,
  );
...
test("module counts read from the SVG agree with bwip-js's own encoder", () => {
  for (const text of ["CGH-P000001", "SMP-000001", "OGH-P123456", "A1"]) {
    const [symbol] = bwipjs.raw({ bcid: "code128", text }) as unknown as {
      sbs: number[];
    }[];
    const expected = symbol.sbs.reduce((a, b) => a + b, 0);
    assert.equal(
      code128(text, { heightMm: 10, availableMm: 200, maxDots: 3 }).modules,
      expected,
      text,
    );
  }
  for (const text of [
    "HMS1|P|CGH-P000001",
    "HMS1|S|SMP-000001",
    "HMS1|P|A-LONGER-HOSPITAL-P000123",
  ]) {
    const [symbol] = bwipjs.raw({ bcid: "datamatrix", text }) as unknown as {
      pixx: number;
      pixy: number;
    }[];
    assert.equal(
      dataMatrix(text, { availableMm: 200, maxDots: 3 }).modules,
      Math.max(symbol.pixx, symbol.pixy),
      text,
    );
  }
});
...
test("DataMatrix modules are square and sized from the module count", () => {
  const sym = dataMatrix(patientPayload("CGH-P000001"), {
    availableMm: 22,
    maxDots: 4,
  });
  assert.ok(sym.fits);
...
test("the payload the label carries is the payload the server reads", async () => {
  const { parseScanCode } = (await import(
    pathToFileURL(serverScanCode).href
  )) as {
  ...
  assert.deepEqual(parseScanCode(patientPayload("CGH-P000001")), {
    ok: true,
    kind: "patient",
    id: "CGH-P000001",
  });
  assert.deepEqual(parseScanCode(specimenPayload("SMP-000123")), {
    ok: true,
    kind: "specimen",
    id: "SMP-000123",
  });
...
test("every interpolated value is escaped; only markup this code built is not", () => {
  ...
  const out = html`<p title="${'"><script>'}">
    ${"<5 mmol/L"}${raw("<b>ok</b>")}${[html`<i>${"<"}</i>`]}
  </p>`.__html;
  assert.equal(
    out,
    `<p title="&quot;&gt;&lt;script&gt;">&lt;5 mmol/L<b>ok</b><i>&lt;</i></p>`,
  );
});

// ---- Content rules ------------------------------------------------------------
test("allergy status is three statements, and none of them is blank", () => {
  assert.deepEqual(allergyStatement({ recorded: false, allergies: [] }), {
    state: "unrecorded",
    text: "Allergies not recorded",
  });
  assert.deepEqual(allergyStatement({ recorded: true, allergies: [] }), {
    state: "none",
  ...
  assert.equal(
    allergyStatement({
      recorded: true,
      allergies: [{ substance: "Sulfa", severity: "mild" }],
    }).text,
    "ALLERGIES: Sulfa",
  );
});
...
test("names print SURNAME first; an unidentified patient prints as unidentified", () => {
  assert.deepEqual(printedName("Anita", "Case"), {
    surname: "CASE",
    given: "Anita",
    unidentified: false,
    visitNumber: null,
  });
  assert.deepEqual(printedName("Unidentified", "ED-000042"), {
    surname: "UNIDENTIFIED",
    given: "",
    unidentified: true,
    visitNumber: "ED-000042",
  });
