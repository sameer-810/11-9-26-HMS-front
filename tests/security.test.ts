...
import {
  ...
  isSafeHtml,
  raw,
} from "../src/shared/print/escapeHtml";
...
import {
  ...
  isTainted,
} from "../src/shared/offline/mirrorPolicy";
import {
  belongsTo,
  ownerOf,
  sameOwner,
} from "../src/shared/offline/outboxOwnership";
...
test("hostile text in every prescription field is inert, in text and attribute contexts", () => {
  ...
  assert.ok(
    !doc.includes(HOSTILE) && !doc.includes(`"><img`) && !doc.includes(`'><`),
    "no attribute can be broken out of",
  );
...
const cache = [
  {
    key: ["medical-record", PATIENT],
    data: { access: { viaBreakGlass: true }, patient: { id: PATIENT } },
  },
  {
    key: ["bedside", ADMISSION],
    data: {
      admission: { id: ADMISSION, patient: { id: PATIENT } },
      observations: [],
    },
  },
  ...
  {
    key: ["bedside", "a-other"],
    data: { admission: { id: "a-other", patient: { id: "p-other" } } },
  },
...
test("a break-the-glass read taints the patient and their admissions, not the ward", () => {
  ...
  const kept = cache
    .filter((e) => !isTainted(e, tainted))
    .map((e) => e.key.join("/"));
  assert.deepEqual(kept, [
    "bedside/a-other",
    "drug-round/a-other/today",
    "my-ward-patients",
  ]);
});
...
test("the taint outlives the emergency read leaving the cache, and nothing is tainted without one", () => {
  ...
  assert.ok(
    isTainted({ key: ["nursing-notes", ADMISSION], data: [] }, tainted),
  );
...
test("a queued op is filed only under the user and hospital that charted it", () => {
  ...
  assert.equal(
    belongsTo(
      { userId: "", hospitalId: "h-1" },
      ownerOf({ id: "", hospitalId: "h-1" }),
    ),
    false,
  );
});
...
test("ops queued before the hospital was recorded still reach their own user", () => {
  assert.equal(
    belongsTo({ userId: "u-a" }, ownerOf({ id: "u-a", hospitalId: "h-1" })),
    true,
  );
  assert.equal(
    belongsTo({ userId: "u-a" }, ownerOf({ id: "u-b", hospitalId: "h-1" })),
    false,
  );
});
